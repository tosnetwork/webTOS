...
- **Hot-block translation** landed (p-code→wasm, gated bit-for-bit, dispatched
  in the run loop natively and in the browser; 2.76x measured on a compute
  loop). What remains is the two levers the coverage histogram scoped: region
  compilation for scalar hot loops (the per-dispatch overhead cap), and the
  128-bit move/widen quartet for the glibc/SIMD class.
...
- Persist the lift cache across sessions, keyed by image content and validated
  ...
  be)
- Translate proven hot paths to WebAssembly. ✅ for the mechanism; 🔶 for the
  speedup ceiling. A p-code→wasm translator (`x64-engine/src/jit.rs`) handles
  ...
  2.76x on a compute loop in V8. Two levers remain, and they belong to
  different workload classes, which the bail-cause histogram (executed-weight)
  settled: for scalar static-musl compute, coverage is already ~100%, so
  region/trace compilation — a hot loop's back-edge folded into one wasm
  function, so N iterations are one `jit_call`, not N — is the lever; for
  glibc/Node, ~9% of executed work bails on 128-bit SIMD ops, so the 128-bit
  move/widen/load quartet is the coverage worth adding there)
