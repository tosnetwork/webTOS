...
| M9 Ice Lake execution parity | ⬜ | 0% | Follow-on ISA milestone: coherent CPUID/XCR0, YMM/ZMM and opmask state, EVEX execution, XSAVE/XRSTOR and signal-state preservation, plus a fail-closed native bit-for-bit differential authority. The plan and parallel work packages are in `docs/M9_ICE_LAKE_EXECUTION_PARITY.md`; no AVX-family feature bit is enabled by this planning change |
...
M9 is a follow-on ISA-expansion program and is tracked separately at 0%; it
...
This is deliberately separate from M7. Forcing simdutf's `icelake` object in
the pinned Claude/Bun fixture currently decodes an EVEX `vpbroadcastd` at RIP
`0x399b8e2`, followed by `vpternlogd`, after 142,435,521 retired guest
instructions. Their opaque p-code operations have no execution helpers, so the
VM reports `IllegalInstruction`. That is evidence of broad
Bun/JavaScriptCore AVX-512 dispatch, not a request to patch two opcodes or
publish speculative CPUID bits.

The milestone is split so CPUID/XCR0, YMM/ZMM and opmask state, EVEX semantics,
XSAVE/XRSTOR plus signal preservation, and the native bit-for-bit oracle can be
investigated in parallel. Feature publication is last. The complete scope,
dependency graph, test authority, and exit gates are in
