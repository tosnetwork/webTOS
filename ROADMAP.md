...
| M8 Performance & release | 14% | 88% | 12.3 |
| **Total** | **100%** | | **94.0** |
...
- **Hot-block translation** landed (p-code→wasm, gated bit-for-bit, dispatched
  ...
  one-per-page. What remains: the 128-bit *arithmetic* ops (IntMul/shift at
  width 16, the cross-lane residue the quartet cannot decompose), and true
  multi-block regions (a relooper).
...
- Translate proven hot paths to WebAssembly. ✅ mechanism, and ✅ the speedup
  ...
  ~30x; only low-value residue is left (cross-lane 128-bit arithmetic,
  multi-block regions). A p-code→wasm translator (`x64-engine/src/jit.rs`) handles
  ...
  aliasing all matched the interpreter. The residue, each measured: the
  cross-lane 128-bit arithmetic ops (IntMul/shift at width 16); and true
  multi-block regions, which need a relooper)
