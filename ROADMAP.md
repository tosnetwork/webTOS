...
- hot-block translation to WebAssembly, landed after the correctness gates (dispatched in the run loop, native and browser; region compilation of register and host self-loops, ~30x and ~4.6x, plus the 128-bit move/widen/logic quartet)
...
- Translate proven hot paths to WebAssembly. ✅ for the mechanism; 🔶 for the
  ...
  glibc/Node coverage from ~90 to ~94% and codex to ~98%. The residue, each now
  measured: an inline softmmu fast path (a TLB in shared memory, checked in wasm,
  calling back only on a miss — that per-access callback is why the memory loop
  is 4.6x not 30x); the cross-lane 128-bit arithmetic ops (IntMul/shift at width
  16); and true multi-block regions, which need a relooper)
