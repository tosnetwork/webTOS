...
- Audit credential boundaries, host messages, guest memory, and snapshot
  ...
  kept the version it replaced. Both are fixed. What is not audited: whether
  a credential can be read out of guest memory by another process in the
  same machine, and what a crash bundle carries beyond the secrets it already
  redacts)
