...
41-check-per-engine browser matrix; `crates/x64-engine` and `crates/linux-compat`
...
- Version and help commands run from a clean browser profile. 🔶 (Codex ✅:
  the real 256 MB standalone installs by content-addressed manifest into a
  clean profile and answers `--version` and `--help` in Chromium, Firefox, and
  WebKit, demand-paging just 21 MiB — 8% of the image — with the retired
  instruction count identical on every engine. Claude Code, a dynamically
  linked Bun runtime needing its loader and libraries delivered, is the
  remaining half)
