...
- hot-block translation to WebAssembly, landed after the correctness gates (dispatched in the run loop, native and browser; region compilation of hot self-loops measured ~26x)
...
- Translate proven hot paths to WebAssembly. ✅ for the mechanism; 🔶 for the
  ...
  2.76x on a compute loop in V8 per block, then region compilation of hot
  self-loops — a loop's back-edge folded into one wasm function, so millions of
  iterations are one `jit_call`, not one each — took the same loop to ~26x. Two
  further levers remain, split by workload class per the bail-cause histogram
  (executed-weight): for scalar static-musl compute coverage is already ~100%,
  so cross-block and host-self-loop regions extend the win; for glibc/Node ~9%
  of executed work bails on 128-bit SIMD ops, so the 128-bit move/widen/load
  quartet is the coverage worth adding there)
