...
**Updated 2026-08-30.** Legend: ✅ complete (gated by tests), 🔶 partial,
⬜ not started.
...
M9 is a follow-on ISA-expansion program and is tracked separately at ~99%; it
does not retroactively change the M0-M8 product-delivery score above. It is not
a prerequisite for closing M7 through the conservative SSE/Westmere route.
...
- Port `clone`, `clone3`, thread groups, `fork`, `vfork`, `execve`, `wait4`,
  and process exit semantics onto virtual CPU contexts. ✅ (`clone3` versions
  0-2 normalize into the legacy scheduler representation; unsupported PID-fd,
  explicit-PID, cgroup, and non-zero future extensions fail closed)
...
all verified against native intrinsics), and initially advertising an SSE2
CPUID baseline. M9 later validated and published the narrower Ice Lake-class
AVX-family profile; this paragraph records the M7 route that first made Node
run. See `docs/workloads/node.md`.
...
- Release artifacts are reproducible and carry complete dependency metadata.
  ...
  The canonical builder is fixed to Ubuntu 24.04 x86-64 Linux, isolates caller
  Cargo configuration, and records that host in its metadata. The current
  compatibility publication passes 45/45 checks in each
...
**Outcome:** webTOS exposes a coherent Ice Lake-class userspace execution
profile whose AVX, AVX2, and selected AVX-512 behavior matches native x86-64
architectural results, including extended register state, faults, save/restore,
and Linux signal transitions.

This is deliberately separate from M7. The original forced simdutf `icelake`
reduction reached an EVEX `vpbroadcastd` at RIP `0x399b8e2`, followed by
`vpternlogd`, after 142,435,521 retired guest instructions and failed because
their p-code operations had no execution helpers. M9 used those instructions
as canaries, then closed the complete profile boundary rather than patching two
opcodes or publishing speculative CPUID bits.

The implementation now includes CPUID/XCR0, all YMM/ZMM and opmask state,
XSAVE/XRSTOR plus real Linux signal preservation, systematic VEX/AVX2/EVEX
semantics, a twice-stable ptrace native authority, and a portable native/JIT/
browser replay. Feature publication was the final implementation step. The
complete scope, dependency graph, test authority, and exit gates are in
[`docs/M9_ICE_LAKE_EXECUTION_PARITY.md`](docs/M9_ICE_LAKE_EXECUTION_PARITY.md).

Implementation work:

- ✅ **M9-A/B — CPUID adapter and named profile.** Complete register ordering,
  finite leaf/subleaf boundaries, the dependency graph, and the exact simdutf
  Ice Lake selector tuple are gated.
- ✅ **M9-C — wide architectural state.** All XMM/YMM/ZMM aliases, K0-K7,
  reset behavior, upper-lane rules, and width-safe helper boundaries are
  covered.
- ✅ **M9-D — XCR0 policy.** XCR0 initializes to `0xe7`; XGETBV selector rules
  and user-mode XSETBV faults are implemented without partial mutation.
- ✅ **M9-E/F — FXSAVE/FXRSTOR and XSAVE/XRSTOR.** Standard layouts,
  requested masks, init-state behavior, complete round trips, reserved bytes,
  alignment, MXCSR, and precise page-crossing faults are covered.
- ✅ **M9-G/H — Linux signal xstate.** A real guest-visible `rt_sigframe`
  saves and restores GPRs, signal state, legacy fp/SSE, YMM, opmask, and ZMM;
  lawful edits made by a compiled handler survive `rt_sigreturn`.
- ✅ **M9-I — native authority.** The ptrace oracle compares GPRs,
  `NT_X86_XSTATE`, masks, memory, and faults, requires two stable native runs,
  detects deliberately corrupted results, and publishes core digest
  `bee2b17f0b247fde` plus a portable replay ELF.
- ✅ **M9-J/K — VEX/AVX/AVX2/EVEX execution.** Systematic register, memory,
  width, mask, zero/merge, tuple, VSIB, floating-point, and partial-fault cases
  include the Claude/Bun canaries. BMI1 TZCNT now uses one width-aware p-code
  operation across the interpreter and wasm JIT.
- ✅ **M9-L — feature publication and integration.** The Ice Lake profile is
  enabled only after the feature ledger reached 789 published mnemonics with
  zero reachable opaque operations lacking helpers. Portable native,
  interpreter, JIT, and browser replay use the same digest-pinned artifact;
  the Linux substrate exposes coherent topology, invariant TSC, CPU clocks,
  preemption, stable per-thread `rseq(2)` registration, and the required
  scheduler subset.
- ✅ **Runtime wake correctness found by the live gate.** A woken untimed
  `FUTEX_WAIT` now completes with zero instead of replaying the syscall and
  incorrectly returning `EAGAIN`; the dedicated process/thread regression is
  green together with all 31 tests in that suite.

Remaining acceptance work:

- 🔶 Rebuild from the current commit and rerun the strict Rust, native-oracle,
  feature-ledger, Clippy, formatting, and three-browser matrix after the futex
  correction.
- 🔶 Complete the real Claude Code browser task under the published Ice Lake
  profile: paint the interactive UI, authenticate through the scoped secret
