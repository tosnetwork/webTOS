...
- Translate proven hot paths to WebAssembly. ✅ for the mechanism; 🔶 for the
  ...
  one-per-byte to one-per-page. The residue, each measured: the cross-lane
  128-bit arithmetic ops (IntMul/shift at width 16); and true multi-block
  regions, which need a relooper)
