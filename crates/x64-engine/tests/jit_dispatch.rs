...
impl JitBackend for WasmiJit {
    ...
    fn call_region(&mut self, handle: u32, cpu: &mut Cpu, max_iters: u64) -> RegionOutcome {
        ...
        // A host region can fault mid-loop: the load/store/raise callback set
        // the exception and recorded the resume index in the fault tracker,
        // exactly as on the per-block path. If it fired, `iters` is the count of
        // fully completed iterations and the tracker is the faulting index. A
