//! Low-level byte reading, LEB128 decoding, and UTF-8 validation.

use crate::wasm::types::WasmError;

pub fn read_byte(bytes: &[u8], pos: &mut usize) -> Result<u8, WasmError> {
    if *pos >= bytes.len() {
...
pub fn peek_byte(bytes: &[u8], pos: usize) -> Result<u8, WasmError> {
    if pos >= bytes.len() {
        return Err(WasmError::UnexpectedEnd);
    }
    Ok(bytes[pos])
...
pub fn validate_utf8(bytes: &[u8]) -> Result<(), WasmError> {
...
pub fn read_name<'a>(bytes: &'a [u8], pos: &mut usize) -> Result<&'a [u8], WasmError> {
