//! Function body type checking — the Validator struct and instruction-level validation.

use alloc::vec;
use alloc::vec::Vec;
use alloc::collections::BTreeSet;
use crate::wasm::decoder::{WasmModule, FuncDef, FuncTypeDef, GcTypeDef, StorageType, ImportKind, ElemMode, ExportKind};
use crate::wasm::types::{ValType, WasmError, MAX_LOCALS, MAX_PARAMS, MAX_RESULTS};
use super::{
    is_subtype, val_is_subtype, is_ref_type, is_non_nullable_ref,
    ref_types_compatible, table_elem_type, table_index_type, is_type_index_subtype,
};
...
impl<'a> Validator<'a> {
    ...
    fn pop_ctrl(&mut self) -> Result<CtrlFrame, WasmError> {
        ...
        let frame = self.ctrl_stack.pop().ok_or(WasmError::StackUnderflow)?;
        Ok(frame)
    }
    ...
    fn resolve_heap_type(&self, heap_type: i32, nullable: bool) -> Result<ValType, WasmError> {
        match heap_type {
            ...
            _ => {
                ...
                Ok(if nullable { ValType::NullableTypedFuncRef } else { ValType::TypedFuncRef })
            }
        }
    }
                ...
                0x1B => {
                    ...
                    match (t1, t2) {
                        (StackType::Known(a), StackType::Known(b)) => {
                            ...
                            if matches!(a, ValType::V128 | ValType::FuncRef | ValType::ExternRef | ValType::TypedFuncRef | ValType::NonNullableFuncRef | ValType::NullableTypedFuncRef) {
                                return Err(WasmError::TypeMismatch);
                            }
                            self.push_val(a);
                        }
                        ...
                        (StackType::Unknown, StackType::Known(b)) => {
                            if matches!(b, ValType::V128 | ValType::FuncRef | ValType::ExternRef | ValType::TypedFuncRef | ValType::NonNullableFuncRef | ValType::NullableTypedFuncRef) {
                                return Err(WasmError::TypeMismatch);
                            }
                            self.push_val(b);
                        }
                ...
                0xFB => {
                    ...
                    match sub {
                        ...
                        2 | 3 | 4 => { // struct.get/get_s/get_u: typeidx fieldidx — pop ref, push val
                            let type_idx = self.read_u32()?; let field_idx = self.read_u32()? as usize;
                            let _ = self.pop_opd()?;
                            // Determine the field type from the GC type definition
                            let field_vt = if let Some(crate::wasm::decoder::GcTypeDef::Struct { ref field_types, .. }) = self.module.gc_types.get(type_idx as usize) {
                            ...
                            // Check src and dst array element types are compatible
                            {
                                ...
                                if let (Some(dst_st), Some(src_st)) = (dst_elem, src_elem) {
                                    ...
                                    let compatible = match (&src_st, &dst_st) {
                                        ...
                                        (StorageType::RefType(_, si), StorageType::RefType(_, di)) => {
                                            if si == di { true }
                                            else {
                                        ...
                                        (StorageType::RefType(sv, _), StorageType::Val(dv)) => val_is_subtype(*sv, *dv),
                                        (StorageType::Val(sv), StorageType::RefType(dv, _)) => val_is_subtype(*sv, *dv),
                                        _ => false,
                                    };
...
pub(super) fn validate_function_body(
    ...
    let param_count_clamped = (ft.param_count as usize).min(MAX_PARAMS);
    for i in 0..param_count_clamped {
        local_types.push(ft.params[i]);
    }
    let declared_locals = (func.local_count as usize).min(MAX_LOCALS);
    for i in 0..declared_locals {
        local_types.push(func.locals[i]);
    }

    let result_count_clamped = (ft.result_count as usize).min(MAX_RESULTS);
    let return_types: Vec<ValType> = ft.results[..result_count_clamped].to_vec();
