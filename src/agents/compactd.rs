...
pub extern "C" fn compactd_main() -> ! {
    serial_println!(
        "[COMPACTD] State compaction service started (interval: {} ticks, threshold: {} entries)",
        COMPACT_INTERVAL,
        HISTORY_THRESHOLD
    );
    ...
    loop {
        ...
        if tick_counter % COMPACT_INTERVAL == 0 {
            ...
            if compacted > 0 {
                ...
                serial_println!(
                    "[COMPACTD] Compacted {} keyspace(s) (pass #{})",
                    compacted,
                    unsafe { TOTAL_PASSES }
                );
            }
...
fn handle_compact_now(sender_id: AgentId) {
    serial_println!(
        "[COMPACTD] On-demand compaction requested by agent {}",
        sender_id
    );
    let compacted = compact_keyspaces();
    unsafe {
    ...
    serial_println!(
        "[COMPACTD] On-demand compaction done: {} keyspace(s)",
        compacted
    );
...
fn handle_compact_status(sender_id: AgentId) {
    ...
    serial_println!(
        "[COMPACTD] Status: {} passes, {} total keyspaces compacted",
        passes,
        total
    );
    ...
    for ks_id in 0..MAX_AGENTS {
        if crate::state::get_version(ks_id as u16).is_some() {
            if crate::state::compact_keyspace_history(ks_id as u16, HISTORY_KEEP) {
                serial_println!(
                    "[COMPACTD] Keyspace {} history trimmed to {} entries",
                    ks_id,
                    HISTORY_KEEP
                );
                compacted += 1;
