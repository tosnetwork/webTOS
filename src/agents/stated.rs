//! TOS stated — State Persistence Manager
//!
...
#[inline(never)]
fn is_get_op(op: u8) -> bool {
    op == OP_GET
}

#[inline(never)]
fn is_put_op(op: u8) -> bool {
    op == OP_PUT
}

#[inline(never)]
fn is_create_op(op: u8) -> bool {
    op == OP_CREATE
}
            ...
            if is_get_op(op) {
                ...
                if msg_len >= 11 {
                    ...
                    if let Some(data) = crate::persist::get(keyspace_id, key) {
                        ...
                        resp_buf[2] = 0;
                        serial_println!(
                            "[STATED] GET keyspace={} key={} -> not found",
                            keyspace_id,
                            key
                        );
                    }
                }
            } else if is_put_op(op) {
                ...
                if msg_len >= 13 {
                    ...
                    serial_println!(
                        "[STATED] PUT keyspace={} key={} len={} -> ok={}",
                        keyspace_id,
                        key,
                        value_len,
                        result.is_ok()
