/*
 * test_java_child_execve.c — Child-process smoke for OpenJDK.
 *
 * Build:
 *   gcc -nostdlib -static -Os -s -Wl,-Ttext=0x40000000 -o test_java_child_execve.elf test_java_child_execve.c
 *
 * Runs:
...
void _start(void) {
    ...
    print("[JAVA] launching java child-process smoke\n");
    long ret = sys_execve(path, argv, envp);
    print("[JAVA] child execve returned ");
    print_num(ret);
    print("\n");
    sys_exit(1);
}
