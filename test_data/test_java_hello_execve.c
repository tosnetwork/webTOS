/*
 * test_java_hello_execve.c — Smoke test for classpath-based OpenJDK startup.
 *
 * Build:
 *   gcc -nostdlib -static -Os -s -Wl,-Ttext=0x40000000 -o test_java_hello_execve.elf test_java_hello_execve.c
 *
 * Runs:
 *   execve("/usr/lib/jvm/java-11-openjdk-amd64/bin/java",
 *          ["java", "-Xshare:off", "-XX:-UsePerfData", "-cp", "/usr/lib/atos-tests", "Hello"], envp)
 *
 * Success is observed if the VM prints "ATOS-JAVA-HELLO" and exits cleanly.
 */
...
void _start(void) {
    ...
    print("[JAVA] launching java -Xshare:off -XX:-UsePerfData -cp /usr/lib/atos-tests Hello\n");
