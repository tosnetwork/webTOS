/*
 * test_signal_smoke.c — Minimal dynamic signal-handler smoke test for TOS.
 *
 * Build with musl:
...
static void on_sigusr1(int signum) {
    handled = signum;
    handled_count++;
    static const char msg[] = "TOS-SIGNAL-HANDLER sig=10\n";
...
int main(void) {
    ...
    if (sigpending((sigset_t *)0) != -1 || errno != EFAULT) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigpending-null\n";
        write(1, msg, sizeof(msg) - 1);
        return 10;
    }
    ...
    if (sigaltstack((stack_t *)0, &old_stack) != 0 ||
        ...
        static const char msg[] = "TOS-SIGNAL-FAIL sigaltstack-default\n";
        write(1, msg, sizeof(msg) - 1);
        return 27;
    }
    ...
    if (sigaltstack(&alt_stack, (stack_t *)0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaltstack-set\n";
        write(1, msg, sizeof(msg) - 1);
        return 28;
    }
    ...
    if (sigaltstack((stack_t *)0, &old_stack) != 0 ||
        ...
        static const char msg[] = "TOS-SIGNAL-FAIL sigaltstack-query\n";
        write(1, msg, sizeof(msg) - 1);
        return 29;
    }
    ...
    if (sigaltstack(&alt_stack, (stack_t *)0) != -1 || errno != ENOMEM) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaltstack-small\n";
        write(1, msg, sizeof(msg) - 1);
        return 30;
    }

    if (sigaction(SIGUSR1, &sa, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction\n";
        write(1, msg, sizeof(msg) - 1);
        return 1;
    }
    ...
    if (sigaction(SIGUSR1, 0, &old_sa) != 0 || old_sa.sa_handler != on_sigusr1) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction-oldact\n";
        write(1, msg, sizeof(msg) - 1);
        return 11;
    }
    ...
    if (sigaction(SIGKILL, &sa, 0) != -1 || errno != EINVAL) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction-sigkill\n";
        write(1, msg, sizeof(msg) - 1);
        return 12;
    }

    if (sigemptyset(&set) != 0 || sigaddset(&set, SIGUSR1) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigset\n";
        write(1, msg, sizeof(msg) - 1);
        return 2;
    }

    if (sigemptyset(&kill_only) != 0 || sigaddset(&kill_only, SIGKILL) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigkill-set\n";
        write(1, msg, sizeof(msg) - 1);
        return 14;
    }
    ...
    if (sigprocmask(123, &set, 0) != -1 || errno != EINVAL) {
        static const char msg[] = "TOS-SIGNAL-FAIL bad-how\n";
        write(1, msg, sizeof(msg) - 1);
        return 13;
    }

    if (sigprocmask(SIG_BLOCK, &set, &old_set) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL block\n";
        write(1, msg, sizeof(msg) - 1);
        return 3;
    }

    if (sigismember(&old_set, SIGUSR1) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL old-mask\n";
        write(1, msg, sizeof(msg) - 1);
        return 15;
    }

    if (kill(getpid(), SIGUSR1) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL kill\n";
        write(1, msg, sizeof(msg) - 1);
        return 4;
    }

    if (handled != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL blocked-delivery\n";
        write(1, msg, sizeof(msg) - 1);
        return 5;
    }

    if (sigpending(&pending) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigpending\n";
        write(1, msg, sizeof(msg) - 1);
        return 6;
    }

    if (sigismember(&pending, SIGUSR1) != 1) {
        static const char msg[] = "TOS-SIGNAL-FAIL not-pending\n";
        write(1, msg, sizeof(msg) - 1);
        return 7;
    }

    static const char pending_ok[] = "TOS-SIGNAL-PENDING sig=10\n";
    write(1, pending_ok, sizeof(pending_ok) - 1);

    if (sigprocmask(SIG_UNBLOCK, &set, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL unblock\n";
        write(1, msg, sizeof(msg) - 1);
        return 8;
    }

    if (handled != SIGUSR1) {
        static const char msg[] = "TOS-SIGNAL-FAIL handler-missed\n";
        write(1, msg, sizeof(msg) - 1);
        return 9;
    }

    if (sigprocmask(SIG_SETMASK, &kill_only, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL setmask-kill\n";
        write(1, msg, sizeof(msg) - 1);
        return 16;
    }

    if (sigprocmask(SIG_SETMASK, (sigset_t *)0, &mask_check) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL getmask\n";
        write(1, msg, sizeof(msg) - 1);
        return 17;
    }

    if (sigismember(&mask_check, SIGKILL) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigkill-blocked\n";
        write(1, msg, sizeof(msg) - 1);
        return 18;
    }

    if (sigemptyset(&set) != 0 || sigprocmask(SIG_SETMASK, &set, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL clear-mask\n";
        write(1, msg, sizeof(msg) - 1);
        return 19;
    }
    ...
    if (sigaction(SIGUSR1, &nested_sa, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction-nodefer\n";
        write(1, msg, sizeof(msg) - 1);
        return 20;
    }

    if (kill(getpid(), SIGUSR1) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL kill-nodefer\n";
        write(1, msg, sizeof(msg) - 1);
        return 21;
    }

    if (handled != SIGUSR1 || handled_count != 2 || nested_seen != 1) {
        static const char msg[] = "TOS-SIGNAL-FAIL nodefer-delivery\n";
        write(1, msg, sizeof(msg) - 1);
        return 22;
    }
    ...
    nested_seen = 0;
    if (sigaction(SIGUSR1, &nested_sa, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction-resethand\n";
        write(1, msg, sizeof(msg) - 1);
        return 23;
    }

    if (kill(getpid(), SIGUSR1) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL kill-resethand\n";
        write(1, msg, sizeof(msg) - 1);
        return 24;
    }

    if (handled != SIGUSR1 || handled_count != 1) {
        static const char msg[] = "TOS-SIGNAL-FAIL resethand-delivery\n";
        write(1, msg, sizeof(msg) - 1);
        return 25;
    }

    if (sigaction(SIGUSR1, 0, &old_sa) != 0 || old_sa.sa_handler != SIG_DFL) {
        static const char msg[] = "TOS-SIGNAL-FAIL resethand-state\n";
        write(1, msg, sizeof(msg) - 1);
        return 26;
    }

    if (sigaction(SIGUSR2, &alt_sa, 0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaction-onstack\n";
        write(1, msg, sizeof(msg) - 1);
        return 31;
    }
    ...
    if (kill(getpid(), SIGUSR2) != 0 || altstack_seen != SIGUSR2) {
        static const char msg[] = "TOS-SIGNAL-FAIL onstack-delivery\n";
        write(1, msg, sizeof(msg) - 1);
        return 32;
    }
    ...
    if (sigaltstack(&alt_stack, (stack_t *)0) != 0) {
        static const char msg[] = "TOS-SIGNAL-FAIL sigaltstack-disable\n";
        write(1, msg, sizeof(msg) - 1);
        return 33;
    }
