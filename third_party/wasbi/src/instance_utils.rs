...
use crate::decoder::{ExportKind, FuncDef, FuncTypeDef, ImportKind, WasmModule};
...
pub fn nth_function_import(
    module: &WasmModule,
    func_idx: u32,
) -> Option<&crate::decoder::ImportDef> {
    let mut seen = 0u32;
...
pub fn find_or_add_func_type(module: &mut WasmModule, ft: &FuncTypeDef) -> u32 {
    for (i, existing) in module.func_types.iter().enumerate() {
        if existing.param_count == ft.param_count
            ...
            && existing.params[..existing.param_count as usize]
                == ft.params[..ft.param_count as usize]
            && existing.results[..existing.result_count as usize]
                == ft.results[..ft.result_count as usize]
        {
            return i as u32;
        }
    }
    ...
    if (func_idx as usize) >= src_import_count {
        ...
        if local_idx < src_module.functions.len() {
            ...
            if code_start + code_len <= src_module.code.len() {
                host_module
                    .code
                    .extend_from_slice(&src_module.code[code_start..code_start + code_len]);
            }
