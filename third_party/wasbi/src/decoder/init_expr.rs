//! Init expression evaluation, scanning, and skipping.

use super::gc_types::GcTypeDef;
use super::reader::*;
use super::InitExprInfo;
use crate::types::*;
    ...
    while p < bytes.len() {
        ...
        match b {
            ...
            0x41 => {
                ...
                if sp < 16 {
                    type_stack[sp] = Some(ValType::I32);
                    sp += 1;
                }
            }
            0x42 => {
                ...
                if sp < 16 {
                    type_stack[sp] = Some(ValType::I64);
                    sp += 1;
                }
            }
            0x43 => {
                p += 4;
                if sp < 16 {
                    type_stack[sp] = Some(ValType::F32);
                    sp += 1;
                }
            }
            0x44 => {
                p += 8;
                if sp < 16 {
                    type_stack[sp] = Some(ValType::F64);
                    sp += 1;
                }
            }
            0xD0 => {
            ...
            0xD2 => {
                ...
                if sp < 16 {
                    type_stack[sp] = Some(ValType::TypedFuncRef);
                    sp += 1;
                }
            }
            0xFD => {
                ...
                p += 16;
                if sp < 16 {
                    type_stack[sp] = Some(ValType::V128);
                    sp += 1;
                }
            }
            0xFB => {
                ...
                if let Ok(sub) = decode_leb128_u32(bytes, &mut p) {
                    match sub {
                        ...
                        1 => {
                            // struct.new_default: pop 0, push ref
                            let _ = decode_leb128_u32(bytes, &mut p);
                            if sp < 16 {
                                type_stack[sp] = None;
                                sp += 1;
                        ...
                        6 => {
                            // array.new: pop init_val + length, push ref
                            let _ = decode_leb128_u32(bytes, &mut p);
                        ...
                        7 => {
                            // array.new_default: pop length, push ref
                            let _ = decode_leb128_u32(bytes, &mut p);
                        ...
                        8 => {
                            // array.new_fixed: pop N values, push ref
                            let _ = decode_leb128_u32(bytes, &mut p);
                        ...
                        9 | 10 => {
                            // array.new_data/elem: pop offset + length, push ref (NOT const-valid)
                        ...
                        15 => {
                            info.has_non_const = true; // array.len: pop ref, push i32
                            if sp > 0 {
                                type_stack[sp - 1] = Some(ValType::I32);
                            }
                        }
                        16 => {
                            info.has_non_const = true;
                        ...
                        24 | 25 => {
                            info.has_non_const = true; // br_on_cast / br_on_cast_fail
                        ...
                        28 => {
                            // ref.i31: pop i32, push i31ref (const-valid)
                            if sp > 0 {
                                type_stack[sp - 1] = Some(ValType::I31Ref);
                            } else if sp < 16 {
                                type_stack[sp] = Some(ValType::I31Ref);
                                sp += 1;
                            }
                        }
                        29 | 30 => {
                            // i31.get_s/u: pop i31ref, push i32 (NOT const-valid)
                            info.has_non_const = true;
                            if sp > 0 {
                                type_stack[sp - 1] = Some(ValType::I32);
                            }
            ...
            0x7C..=0x7E => {
                if sp >= 2 {
                    sp -= 1;
                    type_stack[sp - 1] = Some(ValType::I64);
                }
            }
            _ => {
                info.has_non_const = true;
...
pub fn eval_init_expr_with_globals(
    bytes: &[u8],
    pos: &mut usize,
    globals: &[Value],
...
pub fn eval_init_expr(bytes: &[u8], pos: &mut usize) -> Result<Value, WasmError> {
    eval_init_expr_inner(bytes, pos, None)
}

fn eval_init_expr_inner(
    bytes: &[u8],
    pos: &mut usize,
    ...
    loop {
        ...
        match opcode {
            ...
            0x44 => {
                ...
                *pos += 8;
                if sp < 16 {
                    stack[sp] = Value::F64(f64::from_le_bytes(b8));
                    sp += 1;
                }
            }
            0x23 => {
            ...
            0xFD => {
                ...
                if sub == 12 {
                    ...
                    *pos += 16;
                    if sp < 16 {
                        stack[sp] = Value::V128(crate::types::V128(v));
                        sp += 1;
                    }
                } else {
            ...
            0xFB => {
                ...
                match sub {
                    0 | 1 | 6 | 7 => {
                        decode_leb128_u32(bytes, pos)?; // type_idx
                        ...
                        if sp < 16 {
                            stack[sp] = Value::I32(0);
                            sp += 1;
                        }
                    }
                    8 => {
                        decode_leb128_u32(bytes, pos)?;
                        decode_leb128_u32(bytes, pos)?; // type_idx + count
                        if sp < 16 {
                            stack[sp] = Value::I32(0);
                            sp += 1;
                        }
                    ...
                    9 | 10 => {
                        decode_leb128_u32(bytes, pos)?;
                        decode_leb128_u32(bytes, pos)?; // type_idx + idx
                        if sp < 16 {
                            stack[sp] = Value::I32(0);
                            sp += 1;
                        }
                    ...
                    29 => {
                        // i31.get_s: pop i31ref, sign-extend from 31 bits
                        if sp > 0 {
                            let v = match stack[sp - 1] {
                                Value::I32(v) => v,
                                _ => 0,
                            };
                    ...
                    30 => {
                        // i31.get_u: pop i31ref, mask to 31 bits
                        if sp > 0 {
                            let v = match stack[sp - 1] {
                                Value::I32(v) => v,
                                _ => 0,
            ...
            0x6A => {
                if sp >= 2 {
                    let b = match stack[sp - 1] {
                        Value::I32(v) => v,
                        _ => 0,
                    };
                    let a = match stack[sp - 2] {
                        Value::I32(v) => v,
                        _ => 0,
                    };
                    sp -= 1;
