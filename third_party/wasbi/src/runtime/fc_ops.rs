...
pub(super) fn execute_fc(inst: &mut WasmInstance) -> ExecResult {
    ...
    match sub_opcode {
        ...
        0 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            }
            let a = try_exec!(inst.pop_f32());
            try_exec!(inst.push(Value::I32(sat_trunc_f32_i32(a))));
        }
        1 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f32());
            try_exec!(inst.push(Value::I32(sat_trunc_f32_u32(a) as i32)));
        }
        2 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            }
            let a = try_exec!(inst.pop_f64());
            try_exec!(inst.push(Value::I32(sat_trunc_f64_i32(a))));
        }
        3 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f64());
            try_exec!(inst.push(Value::I32(sat_trunc_f64_u32(a) as i32)));
        }
        4 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f32());
            try_exec!(inst.push(Value::I64(sat_trunc_f32_i64(a))));
        }
        5 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f32());
            try_exec!(inst.push(Value::I64(sat_trunc_f32_u64(a) as i64)));
        }
        6 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f64());
            try_exec!(inst.push(Value::I64(sat_trunc_f64_i64(a))));
        }
        7 => {
            if inst.runtime_class == RuntimeClass::ProofGrade {
                return ExecResult::Trap(WasmError::FloatsDisabled);
            ...
            let a = try_exec!(inst.pop_f64());
            try_exec!(inst.push(Value::I64(sat_trunc_f64_u64(a) as i64)));
        }
        ...
        8 => {
            ...
            if is_dropped {
                ...
                let extra = inst.fuel_state.fuel_costs.bulk_memory_cost(n as u64);
                if !inst.fuel_state.consume_extra(extra) {
                    return ExecResult::OutOfFuel;
                }
                for i in 0..(n as usize) {
                    inst.memory_store.memories[mi][(d as usize) + i] =
                        inst.module.code[seg_data_offset + (s as usize) + i];
                }
            } else {
                return ExecResult::Trap(WasmError::MemoryOutOfBounds);
            }
        ...
        10 => {
            ...
            if dst_mi == src_mi {
                if du <= su {
                    for i in 0..nu {
                        inst.memory_store.memories[dst_mi][du + i] =
                            inst.memory_store.memories[src_mi][su + i];
                    }
                } else {
                    for i in (0..nu).rev() {
                        inst.memory_store.memories[dst_mi][du + i] =
                ...
                let src_bytes: Vec<u8> = inst.memory_store.memories[src_mi][su..su + nu].to_vec();
                inst.memory_store.memories[dst_mi][du..du + nu].copy_from_slice(&src_bytes);
            }
        }
        // memory.fill (11)
        11 => {
            ...
            for i in 0..nu {
                inst.memory_store.memories[mi][du + i] = val;
            }
        }
        ...
        12 => {
            ...
            let is_t64 =
                tbl_idx < inst.module.tables.len() && inst.module.tables[tbl_idx].is_table64;
            ...
            let is_dropped = seg_idx < inst.table_store.dropped_elems.len()
                && inst.table_store.dropped_elems[seg_idx];
            if is_dropped {
                if n != 0 || s != 0 {
                    return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
                }
                if tbl_idx >= inst.table_store.tables.len()
                    || (d as usize) > inst.table_store.tables[inst.tbl(tbl_idx)].len()
                {
                    return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
                }
                ...
                let tbl_len = inst.table_store.tables[inst.tbl(tbl_idx)].len();
                let src_end = (s as u64) + (n as u64);
                let dst_end = (d as u64) + (n as u64);
                if src_end > seg_len as u64 || dst_end > tbl_len as u64 {
                    return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
                }
                let extra = inst.fuel_state.fuel_costs.bulk_table_cost(n as u64);
                if !inst.fuel_state.consume_extra(extra) {
                    return ExecResult::OutOfFuel;
                }
                let rt = resolve_alias(&inst.table_store.table_aliases, tbl_idx);
                for i in 0..(n as usize) {
                    let func_idx =
                        inst.module.element_segments[seg_idx].func_indices[(s as usize) + i];
                    if func_idx == u32::MAX {
                        inst.table_store.tables[rt][(d as usize) + i] = None;
                    } else {
                        inst.table_store.tables[rt][(d as usize) + i] = Some(func_idx);
                    }
                }
            } else {
                return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
        ...
        13 => {
            let seg_idx = try_exec!(inst.read_leb128_u32()) as usize;
            if seg_idx < inst.table_store.dropped_elems.len() {
                inst.table_store.dropped_elems[seg_idx] = true;
            }
        }
        // table.copy (14)
        14 => {
            ...
            let src_t64 =
                src_tbl < inst.module.tables.len() && inst.module.tables[src_tbl].is_table64;
            let dst_t64 =
                dst_tbl < inst.module.tables.len() && inst.module.tables[dst_tbl].is_table64;
            // n: smaller of src/dst (i32 if either is i32)
            let n_is_64 = src_t64 && dst_t64;
            let n = if n_is_64 {
                try_exec!(inst.pop_i64()) as u64
            } else {
                try_exec!(inst.pop_i32()) as u32 as u64
            };
            let s = if src_t64 {
                try_exec!(inst.pop_i64()) as u64
            } else {
            ...
            let su = s as usize;
            let du = d as usize;
            if dst_tbl >= inst.table_store.tables.len() || src_tbl >= inst.table_store.tables.len()
            {
                return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
            }
            if su.saturating_add(nu) > inst.table_store.tables[inst.tbl(src_tbl)].len()
                || du.saturating_add(nu) > inst.table_store.tables[inst.tbl(dst_tbl)].len()
            {
                return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
            }
            let extra = inst.fuel_state.fuel_costs.bulk_table_cost(n);
            if !inst.fuel_state.consume_extra(extra) {
                return ExecResult::OutOfFuel;
            }
            let rd = resolve_alias(&inst.table_store.table_aliases, dst_tbl);
            let rs = resolve_alias(&inst.table_store.table_aliases, src_tbl);
            if rd == rs {
                if du <= su {
                    for i in 0..nu {
                        inst.table_store.tables[rd][du + i] = inst.table_store.tables[rd][su + i];
                    }
                } else {
                    for i in (0..nu).rev() {
                        inst.table_store.tables[rd][du + i] = inst.table_store.tables[rd][su + i];
                ...
                for i in 0..nu {
                    let val = inst.table_store.tables[rs][su + i];
                    inst.table_store.tables[rd][du + i] = val;
                }
            }
        }
        // table.grow (15)
        15 => {
            ...
            // Fuel charged after success check (n can be enormous for failing grows)
            if tbl_idx >= inst.table_store.tables.len() {
                if is_t64 {
                ...
                let old_size = inst.table_store.tables[inst.tbl(tbl_idx)].len() as u64;
                let new_size = old_size + n;
                let max = inst.module.tables.get(tbl_idx).and_then(|t| t.max);
                let module_limit = max.map_or(MAX_TABLE_SIZE as u64, |m| m as u64);
                let limit = module_limit.min(inst.max_table_elements as u64);
                if new_size > limit {
                    if is_t64 {
                        try_exec!(inst.push(Value::I64(-1)));
                    } else {
                    ...
                    // Charge fuel proportional to actual growth
                    let extra = inst.fuel_state.fuel_costs.bulk_table_cost(n);
                    if !inst.fuel_state.consume_extra(extra) {
                        return ExecResult::OutOfFuel;
                    }
                    ...
                    let rt = resolve_alias(&inst.table_store.table_aliases, tbl_idx);
                    inst.table_store.tables[rt].resize(new_size as usize, fill_val);
                    if is_t64 {
                        try_exec!(inst.push(Value::I64(old_size as i64)));
                    } else {
                        try_exec!(inst.push(Value::I32(old_size as i32)));
                    }
                }
            }
        }
        ...
        16 => {
            let tbl_idx = try_exec!(inst.read_leb128_u32()) as usize;
            let size = if tbl_idx < inst.table_store.tables.len() {
                inst.table_store.tables[inst.tbl(tbl_idx)].len()
            } else {
                0
            };
        ...
        17 => {
            ...
            let du = d as usize;
            if tbl_idx >= inst.table_store.tables.len()
                || du.saturating_add(nu) > inst.table_store.tables[inst.tbl(tbl_idx)].len()
            {
                return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
            }
            let extra = inst.fuel_state.fuel_costs.bulk_table_cost(n);
            ...
            let rt = resolve_alias(&inst.table_store.table_aliases, tbl_idx);
            for i in 0..nu {
                inst.table_store.tables[rt][du + i] = entry;
            }
        }
