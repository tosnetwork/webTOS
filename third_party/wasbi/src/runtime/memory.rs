    ...
    match opcode {
        ...
        0x40 => {
            ...
            let config_max = if page_size == 0 {
                ...
                (inst.max_memory_pages * WASM_PAGE_SIZE) / page_size
            };
            let module_max = if mi < inst.module.memories.len()
                && inst.module.memories[mi].max_pages != u32::MAX
            {
                inst.module.memories[mi].max_pages as usize
            } else if inst.module.has_memory_max
                && mi == 0
                && inst.module.memory_max_pages != u32::MAX
            {
            ...
            if new_pages as usize > effective_max {
                ...
                // Charge fuel proportional to actual growth (only on success)
                let extra = inst.fuel_state.fuel_costs.memory_grow_cost(delta as u64);
                ...
                if mi < inst.memory_store.memories.len() {
                    inst.memory_store.memories[mi].resize(new_size, 0);
                    inst.memory_store.memory_sizes[mi] = new_size;
                }
                if is_mem64 {
