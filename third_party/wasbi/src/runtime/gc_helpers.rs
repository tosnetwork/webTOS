...
impl WasmInstance {
    ...
    pub(crate) fn eval_gc_globals(&mut self) {
        ...
        let global_info: Vec<(Vec<u8>, ValType)> = self
            .module
            .globals
            .iter()
        ...
        for (gi, (ref expr_bytes, val_type)) in global_info.iter().enumerate() {
            ...
            if let Some(val) = self.eval_gc_const_expr(expr_bytes, 0) {
                self.global_store.globals[gi] = val;
            }
        }
    }
    ...
    pub(crate) fn eval_gc_elem_exprs(&mut self) {
        ...
        for seg_idx in 0..seg_count {
            ...
            if self.gc_store.elem_gc_values.len() <= seg_idx {
                self.gc_store.elem_gc_values.resize(seg_count, Vec::new());
            }
            self.gc_store.elem_gc_values[seg_idx] = new_values.clone();
            // Update tables for active segments
            if mode == ElemMode::Active && tbl_idx < self.table_store.tables.len() {
                for (i, val) in new_values.iter().enumerate() {
                    let idx = offset + i;
                    if idx < self.table_store.tables[self.tbl(tbl_idx)].len() {
                        let entry = match val {
                            Value::NullRef => None,
                            Value::I32(v) => {
                                if *v < 0 {
                        ...
                        let rt = resolve_alias(&self.table_store.table_aliases, tbl_idx);
    ...
    pub(crate) fn eval_gc_const_expr(&mut self, bytes: &[u8], start: usize) -> Option<Value> {
        use crate::decoder::{decode_leb128_i32, decode_leb128_i64, decode_leb128_u32};
        ...
        loop {
            ...
            match opcode {
                ...
                0x41 => {
                    if let Ok(v) = decode_leb128_i32(bytes, &mut pos) {
                        stack.push(Value::I32(v));
                    }
                }
                0x42 => {
                    if let Ok(v) = decode_leb128_i64(bytes, &mut pos) {
                    ...
                    if let Ok(sub) = decode_leb128_u32(bytes, &mut pos) {
                        match sub {
                            0 => {
                                ...
                                if let Ok(type_idx) = decode_leb128_u32(bytes, &mut pos) {
                                    ...
                                    for i in 0..field_count {
                                        fields[i] =
                                            self.gc_wrap_field_value(type_idx, i, fields[i]);
                                    }
                                    let heap_idx = self.gc_store.gc_heap.len() as u32;
                                    self.gc_store
                                        .gc_heap
                                        .push(GcObject::Struct { type_idx, fields });
                            ...
                            6 => {
                                ...
                                if let Ok(type_idx) = decode_leb128_u32(bytes, &mut pos) {
                                    let length =
                                        stack.pop().map(|v| v.as_i32() as u32).unwrap_or(0);
                                    ...
                                    self.gc_store
                                        .gc_heap
                                        .push(GcObject::Array { type_idx, elements });
                                    stack.push(Value::GcRef(heap_idx));
                                }
                            }
                            7 => {
                            ...
                            29 => {
                                ...
                                if let Some(val) = stack.last_mut() {
                                    ...
                                    *val = Value::I32(if v & 0x4000_0000 != 0 {
                                        v | !0x7FFF_FFFFu32 as i32
                                    } else {
                                        v
                                    });
                                }
                ...
                0x6A => {
                    if let (Some(b), Some(a)) = (stack.pop(), stack.pop()) {
                        stack.push(Value::I32(a.as_i32().wrapping_add(b.as_i32())));
                    }
                }
                0x6B => {
                    if let (Some(b), Some(a)) = (stack.pop(), stack.pop()) {
                        stack.push(Value::I32(a.as_i32().wrapping_sub(b.as_i32())));
                    }
                }
                0x6C => {
                    if let (Some(b), Some(a)) = (stack.pop(), stack.pop()) {
                        stack.push(Value::I32(a.as_i32().wrapping_mul(b.as_i32())));
                    }
                }
                _ => return None,
            }
    ...
    pub(crate) fn gc_array_elem_default(&self, type_idx: u32) -> Value {
        if let Some(GcTypeDef::Array { elem_type, .. }) =
            self.module.gc_types.get(type_idx as usize)
        {
    ...
    pub(crate) fn gc_apply_field_extend(
        &self,
        type_idx: u32,
        field_idx: usize,
        val: Value,
    ...
    pub(crate) fn gc_apply_array_extend(
        &self,
        type_idx: u32,
        val: Value,
        sub_opcode: u32,
    ...
    pub(crate) fn gc_array_from_data(
        &self,
        type_idx: u32,
        data_idx: usize,
        offset: u32,
        ...
        let elem_size = if let Some(GcTypeDef::Array { elem_type, .. }) =
            self.module.gc_types.get(type_idx as usize)
        {
            match elem_type {
            ...
            let val = match elem_size {
                ...
                4 => {
                    ...
                    if let Some(GcTypeDef::Array {
                        elem_type: StorageType::Val(ValType::F32),
                        ..
                    }) = self.module.gc_types.get(type_idx as usize)
                    {
                ...
                8 => {
                    ...
                    if let Some(GcTypeDef::Array {
                        elem_type: StorageType::Val(ValType::F64),
                        ..
                    }) = self.module.gc_types.get(type_idx as usize)
                    {
    ...
    pub(crate) fn gc_array_from_elem(
        &self,
        elem_idx: usize,
        offset: u32,
        length: u32,
        ...
        // Dropped segments are treated as having length 0
        let is_dropped = elem_idx < self.table_store.dropped_elems.len()
            && self.table_store.dropped_elems[elem_idx];
        ...
        if elem_idx < self.gc_store.elem_gc_values.len()
            && !self.gc_store.elem_gc_values[elem_idx].is_empty()
        {
            let gc_vals = &self.gc_store.elem_gc_values[elem_idx];
            let mut elements = Vec::with_capacity(length as usize);
    ...
    pub(crate) fn gc_ref_test(&self, val: Value, ht: i32, nullable: bool) -> bool {
        match val {
                    ...
                    _ if ht >= 0 => {
                        ...
                        if v >= 0 {
                            ...
                            let func_type = if (func_idx as usize) < self.module.func_import_count()
                            {
                                ...
                                let li = (func_idx as usize)
                                    .wrapping_sub(self.module.func_import_count());
            ...
            Value::GcRef(heap_idx) => {
                if heap_idx as usize >= self.gc_store.gc_heap.len() {
                    return false;
                }
                let obj = &self.gc_store.gc_heap[heap_idx as usize];
    ...
    pub(crate) fn gc_types_equivalent(&self, type_a: u32, type_b: u32) -> bool {
        ...
        crate::validator::types_equivalent_in_module(&self.module, type_a, type_b)
    }
