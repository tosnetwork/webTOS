...
pub(super) fn execute_gc(inst: &mut WasmInstance) -> ExecResult {
    ...
    match sub {
        ...
        29 => {
            // i31.get_s: pop i31ref, sign-extend from 31 bits, push i32
        ...
        30 => {
            // i31.get_u: pop i31ref, mask to 31 bits, push i32
            let val = try_exec!(inst.pop());
            match val {
                Value::NullRef => {
                    return ExecResult::Trap(WasmError::NullI31Reference);
        ...
        0 => {
            // struct.new: typeidx — pop fields (in reverse), push ref
            let type_idx = try_exec!(inst.read_leb128_u32());
            let field_count = inst.gc_struct_field_count(type_idx);
            let extra = inst.fuel_state.fuel_costs.gc_cost(field_count as u64);
            ...
            let heap_idx = inst.gc_store.gc_heap.len() as u32;
            inst.gc_store
                .gc_heap
        ...
        1 => {
            // struct.new_default: typeidx — push ref with default fields
            let type_idx = try_exec!(inst.read_leb128_u32());
            let field_count = inst.gc_struct_field_count(type_idx);
            let extra = inst.fuel_state.fuel_costs.gc_cost(field_count as u64);
            if !inst.fuel_state.consume_extra(extra) {
        ...
        2..=4 => {
            ...
            if heap_idx >= inst.gc_store.gc_heap.len() {
                return ExecResult::Trap(WasmError::NullStructReference);
            }
            let val = match &inst.gc_store.gc_heap[heap_idx] {
        ...
        5 => {
            // struct.set: typeidx fieldidx — pop value + ref, set field
            let type_idx = try_exec!(inst.read_leb128_u32());
            let field_idx = try_exec!(inst.read_leb128_u32()) as usize;
            let val = try_exec!(inst.pop());
            let ref_val = try_exec!(inst.pop());
            ...
            match &mut inst.gc_store.gc_heap[heap_idx] {
                GcObject::Struct { fields, .. } => {
                    if field_idx >= fields.len() {
                        return ExecResult::Trap(WasmError::OutOfBounds);
                    }
        ...
        6 => {
            // array.new: typeidx — pop init_value + length, allocate, push ref
            let type_idx = try_exec!(inst.read_leb128_u32());
            let length = try_exec!(inst.pop_i32()) as u32;
            let init_val = try_exec!(inst.pop());
            let extra = inst.fuel_state.fuel_costs.gc_cost(length as u64);
            if !inst.fuel_state.consume_extra(extra) {
                return ExecResult::OutOfFuel;
            }
        ...
        7 => {
            // array.new_default: typeidx — pop length, allocate with defaults
            let type_idx = try_exec!(inst.read_leb128_u32());
            let length = try_exec!(inst.pop_i32()) as u32;
            let extra = inst.fuel_state.fuel_costs.gc_cost(length as u64);
            if !inst.fuel_state.consume_extra(extra) {
        ...
        8 => {
            // array.new_fixed: typeidx + count — pop count values, allocate
            let type_idx = try_exec!(inst.read_leb128_u32());
            let count = try_exec!(inst.read_leb128_u32()) as usize;
            let extra = inst.fuel_state.fuel_costs.gc_cost(count as u64);
            if !inst.fuel_state.consume_extra(extra) {
                return ExecResult::OutOfFuel;
            }
        ...
        9 => {
            // array.new_data: typeidx + data_idx — pop offset + length
            let type_idx = try_exec!(inst.read_leb128_u32());
        ...
        10 => {
            // array.new_elem: typeidx + elem_idx — pop offset + length
            let type_idx = try_exec!(inst.read_leb128_u32());
        ...
        14 => {
            // array.set: typeidx — pop value + index + ref, set element
            let type_idx = try_exec!(inst.read_leb128_u32());
            let val = try_exec!(inst.pop());
            let index = try_exec!(inst.pop_i32()) as u32;
            let ref_val = try_exec!(inst.pop());
        ...
        15 => {
            ...
            let len = match &inst.gc_store.gc_heap[heap_idx] {
        ...
        16 => {
            // array.fill: typeidx — pop length + value + offset + ref
            let type_idx = try_exec!(inst.read_leb128_u32());
            let length = try_exec!(inst.pop_i32()) as u32;
            let val = try_exec!(inst.pop());
            let offset = try_exec!(inst.pop_i32()) as u32;
        ...
        17 => {
            ...
            };
            {
                ...
                match &inst.gc_store.gc_heap[dst_idx] {
                    GcObject::Array { elements, .. } => {
                ...
                // Check source bounds
                match &inst.gc_store.gc_heap[src_idx] {
                    GcObject::Array { elements, .. } => {
                ...
                if length > 0 {
                    ...
                    match &mut inst.gc_store.gc_heap[dst_idx] {
                        GcObject::Array { elements, .. } => {
        ...
        18 => {
            // array.init_data: typeidx + data_idx
            let type_idx = try_exec!(inst.read_leb128_u32());
            let data_idx = try_exec!(inst.read_leb128_u32()) as usize;
            let length = try_exec!(inst.pop_i32()) as u32;
            let src_offset = try_exec!(inst.pop_i32()) as u32;
            ...
            match &inst.gc_store.gc_heap[heap_idx] {
                GcObject::Array { elements, .. } => {
                    let dst_end = dst_offset as usize + length as usize;
                    if dst_end > elements.len() {
                        return ExecResult::Trap(WasmError::ArrayOutOfBounds);
            ...
            let src_elems =
                try_exec!(inst.gc_array_from_data(type_idx, data_idx, src_offset, length));
            let extra = inst.fuel_state.fuel_costs.gc_cost(length as u64);
            if !inst.fuel_state.consume_extra(extra) {
                return ExecResult::OutOfFuel;
            }
        ...
        19 => {
            // array.init_elem: typeidx + elem_idx
        ...
        20 | 21 => {
            // ref.test / ref.test null: heaptype
        ...
        22 | 23 => {
            // ref.cast / ref.cast null: heaptype
            let ht = try_exec!(inst.read_leb128_i32());
            let nullable = sub == 23;
            let ref_val = try_exec!(inst.pop());
        ...
        25 => {
            // br_on_cast_fail: flags + label + ht1 + ht2
            let _flags = try_exec!(inst.read_byte());
            let label = try_exec!(inst.read_leb128_u32());
            let _ht1 = try_exec!(inst.read_leb128_i32());
            let ht2 = try_exec!(inst.read_leb128_i32());
        ...
        26 => {
            ...
            match val {
                Value::NullRef | Value::I32(-1) => {
                    try_exec!(inst.push(Value::NullRef));
                }
                _ => {
                    ...
                    inst.gc_store
                        .gc_heap
                        .push(GcObject::Internalized { value: val });
                    try_exec!(inst.push(Value::GcRef(heap_idx)));
                }
            }
        }
        27 => {
            ...
            match val {
                ...
                _ => {
                    ...
                    inst.gc_store
                        .gc_heap
                        .push(GcObject::Externalized { value: val });
                    try_exec!(inst.push(Value::GcRef(heap_idx)));
                }
            }
        }
