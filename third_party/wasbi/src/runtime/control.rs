...
impl WasmInstance {
    ...
    pub(super) fn scan_legacy_try(
        &mut self,
    ) -> Result<([LegacyCatch; MAX_LEGACY_CATCHES], u8, usize, u32), WasmError> {
        let save_pc = self.pc;
    ...
    pub(crate) fn skip_to_end(&mut self) -> Result<usize, WasmError> {
        ...
        while depth > 0 {
            ...
            match b {
                ...
                0x06 => {
                    // try (legacy exception handling) — opens a block
                ...
                0x18 => {
                    // delegate (legacy): label — ends the try block
                    let _ = self.read_leb128_u32()?;
                    depth -= 1;
                    if depth == 0 {
                        return Ok(self.pc);
                ...
                0xFD => {
                    ...
                    match sub {
                        0x00..=0x0b => {
                            // v128 load/store: memarg (flags [+ memidx] + offset)
                ...
                0x43 => {
                    self.pc += 4;
                } // f32.const (4 bytes IEEE 754)
                0x44 => {
                    self.pc += 8;
                } // f64.const (8 bytes IEEE 754)
                0x0F => {} // return
                0x1C => {
                ...
                0xFB => {
                    ...
                    match sub {
                        ...
                        20 | 21 => {
                            let _ = self.read_leb128_i32()?;
                        } // ref.test, ref.test (nullable): heaptype
                        22 | 23 => {
                            let _ = self.read_leb128_i32()?;
                        } // ref.cast, ref.cast (nullable): heaptype
                        24 | 25 => {
                        ...
                        28..=30 => {} // ref.i31, i31.get_s, i31.get_u: no immediates
