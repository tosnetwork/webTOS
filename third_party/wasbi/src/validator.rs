...
use crate::decoder::{ElemMode, ExportKind, ImportKind, WasmModule};
use crate::types::{ValType, WasmError, MAX_MEMORY_PAGES};
...
// INVARIANT: validation never modifies the module (takes &WasmModule, not &mut).
// After validate() returns Ok(()), all function bodies are type-safe: every
// instruction has correct operand types and every control flow path is well-typed.
pub fn validate(module: &WasmModule) -> Result<(), WasmError> {
    ...
    for table in &module.tables {
        ...
        if let Some(ref expr_bytes) = table.init_expr_bytes {
            let info = crate::decoder::scan_init_expr_info(expr_bytes, 0);
    ...
    for (g_idx, global) in module.globals.iter().enumerate() {
        ...
        if let Some(expr_type) = global.init_expr_type {
            if expr_type != global.val_type
                && !global.has_non_const
                && !is_ref_compatible(expr_type, global.val_type)
                && (!module.gc_enabled || !val_is_subtype(expr_type, global.val_type))
            {
                return Err(WasmError::TypeMismatch);
            }
            ...
            if let Some(ref_idx) = global.init_global_ref {
                ...
                if let Some(rt) = ref_type {
                    if rt != global.val_type
                        && !is_ref_compatible(rt, global.val_type)
                        && (!module.gc_enabled || !val_is_subtype(rt, global.val_type))
                    {
                        return Err(WasmError::TypeMismatch);
                    }
                }
        ...
        if module.gc_enabled {
            if let (Some(global_ht), Some(func_idx)) = (global.heap_type, global.init_func_ref) {
                let init_is_funcref = matches!(
                    global.init_expr_type,
                ...
                if global_ht >= 0 && init_is_funcref {
                    ...
                    let fti = if (func_idx as usize) < func_import_count {
                        ...
                        module
                            .functions
                            .get(func_idx as usize - func_import_count)
                            .map(|f| f.type_idx)
                    };
                    if let Some(fti) = fti {
    ...
    for seg in &module.element_segments {
        ...
        if seg.mode == ElemMode::Active {
            ...
            let effective_elem_type =
                if seg.item_expr_infos.is_empty() && seg.elem_type == ValType::FuncRef {
                    ValType::TypedFuncRef
                } else {
                    seg.elem_type
                };
    ...
    for seg in &module.data_segments {
        if seg.is_active {
            ...
            if module.multi_memory_enabled {
                if seg.memory_idx >= module.memory_count {
                    return Err(WasmError::MemoryOutOfBounds);
                }
            } else if seg.memory_idx > 0 {
                return Err(WasmError::MemoryOutOfBounds);
            }
            // Validate offset expression (memory64 uses I64 offset)
            let offset_type = if (seg.memory_idx as usize) < module.memories.len() {
                if module.memories[seg.memory_idx as usize].is_memory64 {
                    ValType::I64
                } else {
                    ValType::I32
                }
            } else if module.is_memory64 {
                ValType::I64
            } else {
                ValType::I32
            };
...
fn count_global_imports(module: &WasmModule) -> usize {
    module
        .imports
        .iter()
        .filter(|imp| matches!(imp.kind, ImportKind::Global(_, _, _)))
        .count()
}
