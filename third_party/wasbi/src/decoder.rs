...
pub use gc_types::{GcTypeDef, StorageType, SubTypeInfo};
pub use init_expr::{eval_init_expr_with_globals, scan_init_expr_info, scan_init_expr_info_gc};
pub use reader::{decode_leb128_i32, decode_leb128_i64, decode_leb128_u32, decode_leb128_u64};
...
pub struct WasmModule {
    ...
    /// Full original WASM binary — needed for re-evaluating offset expressions
    /// that reference imported globals (positions in offset_expr_range point here).
...
impl WasmModule {
    // INVARIANT: all Vec fields start empty, all booleans default to false,
    // all Option fields default to None. The module is not usable until
    // populated by decode().
    pub fn new() -> Self {
        WasmModule {
            func_types: Vec::new(),
    ...
    pub fn get_func_types(&self) -> &[FuncTypeDef] {
        &self.func_types
    }
    /// Mutable function type signatures.
    pub fn get_func_types_mut(&mut self) -> &mut Vec<FuncTypeDef> {
        &mut self.func_types
    }
    /// Local function definitions.
    pub fn get_functions(&self) -> &[FuncDef] {
        &self.functions
    }
    /// Mutable local function definitions.
    pub fn get_functions_mut(&mut self) -> &mut Vec<FuncDef> {
        &mut self.functions
    }
    /// Import definitions.
    pub fn get_imports(&self) -> &[ImportDef] {
        &self.imports
    }
    /// Export definitions.
    pub fn get_exports(&self) -> &[ExportDef] {
        &self.exports
    }
    /// Global definitions.
    ...
    /// Mutable global definitions.
    pub fn get_globals_mut(&mut self) -> &mut Vec<GlobalDef> {
        &mut self.globals
    }
    /// Table definitions.
    pub fn get_tables(&self) -> &[TableDef] {
        &self.tables
    }
    /// Memory definitions.
    pub fn get_memories(&self) -> &[MemoryDef] {
        &self.memories
    }
    /// Data segments.
    pub fn get_data_segments(&self) -> &[DataSegment] {
        &self.data_segments
    }
    /// Mutable data segments.
    pub fn get_data_segments_mut(&mut self) -> &mut Vec<DataSegment> {
        &mut self.data_segments
    }
    /// Element segments.
    pub fn get_element_segments(&self) -> &[ElementSegment] {
        &self.element_segments
    }
    /// Mutable element segments.
    pub fn get_element_segments_mut(&mut self) -> &mut Vec<ElementSegment> {
        &mut self.element_segments
    }
    /// Tag type indices.
    pub fn get_tag_types(&self) -> &[u32] {
        &self.tag_types
    }
    /// GC type definitions.
    pub fn get_gc_types(&self) -> &[GcTypeDef] {
        &self.gc_types
    }
    /// Subtype hierarchy info.
    pub fn get_sub_types(&self) -> &[SubTypeInfo] {
        &self.sub_types
    }
    /// Raw bytecode.
    ...
    /// Full original WASM binary.
    pub fn get_original_bytes(&self) -> &[u8] {
        &self.original_bytes
    }
    /// Mutable raw bytecode.
    pub fn get_code_mut(&mut self) -> &mut Vec<u8> {
        &mut self.code
    }
    /// Name bytes.
    pub fn get_names(&self) -> &[u8] {
        &self.names
    }
    /// Start function index.
    pub fn get_start_func(&self) -> Option<u32> {
        self.start_func
    }

    // ─── Scalar/flag accessors ──────────────────────────────────────────

    /// Whether GC-proposal features are enabled.
    pub fn gc_enabled(&self) -> bool {
        self.gc_enabled
    }
    /// Set GC-proposal enabled.
    pub fn set_gc_enabled(&mut self, v: bool) {
        self.gc_enabled = v;
    }
    /// Whether implicit rec groups are enabled.
    pub fn implicit_rec_enabled(&self) -> bool {
        self.implicit_rec_enabled
    }
    /// Set implicit rec groups enabled.
    pub fn set_implicit_rec_enabled(&mut self, v: bool) {
        self.implicit_rec_enabled = v;
    }
    /// Whether multi-memory proposal is enabled.
    pub fn multi_memory_enabled(&self) -> bool {
        self.multi_memory_enabled
    }
    /// Set multi-memory enabled.
    pub fn set_multi_memory_enabled(&mut self, v: bool) {
        self.multi_memory_enabled = v;
    }
    /// Whether multiple tables should be rejected.
    pub fn reject_multi_table(&self) -> bool {
        self.reject_multi_table
    }
    /// Set reject multi table.
    pub fn set_reject_multi_table(&mut self, v: bool) {
        self.reject_multi_table = v;
    }
    /// Whether the module has a memory section.
    pub fn has_memory(&self) -> bool {
        self.has_memory
    }
    /// Whether the first memory has a max size.
    pub fn has_memory_max(&self) -> bool {
        self.has_memory_max
    }
    /// Minimum pages of the first memory.
    pub fn memory_min_pages(&self) -> u32 {
        self.memory_min_pages
    }
    /// Maximum pages of the first memory.
    pub fn memory_max_pages(&self) -> u32 {
        self.memory_max_pages
    }
    /// Whether the first memory uses 64-bit addressing.
    pub fn is_memory64(&self) -> bool {
        self.is_memory64
    }
    /// Custom page size log2 for the first memory.
    pub fn page_size_log2(&self) -> Option<u32> {
        self.page_size_log2
    }
}
...
pub fn decode(bytes: &[u8]) -> Result<WasmModule, WasmError> {
    ...
    // INVARIANT: non-custom sections must appear in strictly ascending order
    // (type < import < func < table < memory < global < export < start < element
    // < datacount < code < data < tag). Duplicate non-custom sections are rejected.
    // Custom sections (id 0) may appear anywhere without affecting ordering.
    ...
    while pos < bytes.len() {
        ...
        match section_id {
            ...
            SECTION_DATA_COUNT => {
                decode_data_count_section(bytes, &mut pos, section_end, &mut module)?
            }
            SECTION_TAG => {
                ...
                for _ in 0..count {
                    ...
                    if (type_idx as usize) < module.func_types.len()
                        && module.func_types[type_idx as usize].result_count > 0
                    {
        ...
        if section_id != 0 && pos != section_end {
            return Err(WasmError::InvalidSection);
        }
    }
    ...
    // Save the original bytes for re-evaluating offset expressions
    module.original_bytes = bytes.to_vec();
            ...
            match heap_type {
                ...
                -16 => {
                    ...
                    if nullable {
                        ...
                        Ok(ValType::NonNullableFuncRef)
                    }
                }
                -18 => {
                    if nullable {
                        Ok(ValType::NullableAnyRef)
                    } else {
                        Ok(ValType::AnyRef)
                    }
                }
                ...
                -21 => {
                    if nullable {
                        Ok(ValType::NullableStructRef)
                    } else {
                        Ok(ValType::StructRef)
                    }
                }
                -22 => {
                    if nullable {
                        Ok(ValType::NullableArrayRef)
                    } else {
                        Ok(ValType::ArrayRef)
                    }
                }
                ...
                _ if heap_type >= 0 => {
                    ...
                    if nullable {
                        Ok(ValType::NullableTypedFuncRef)
                    } else {
                        Ok(ValType::TypedFuncRef)
                    }
                }
...
fn decode_valtype_gc_aware_with_limit(
    bytes: &[u8],
    pos: &mut usize,
    ...
    if b == 0x63 || b == 0x64 {
        ...
        *pos = saved;
        ...
        if matches!(
            b,
            0x6E | 0x6D | 0x6C | 0x6B | 0x6A | 0x71 | 0x74 | 0x69 | 0x68
        ) {
            module.gc_enabled = true;
        }
        *pos = saved;
...
fn decode_reftype_real_with_limit(
    bytes: &[u8],
    pos: &mut usize,
    max_type_idx: u32,
) -> Result<ValType, WasmError> {
...
fn decode_storage_type_with_limit(
    bytes: &[u8],
    pos: &mut usize,
    module: &mut WasmModule,
    max_type_idx: u32,
    ...
    match inner_marker {
        0x60 => {
            ...
            for p in 0..param_count as usize {
                ft.params[p] =
                    decode_valtype_gc_aware_with_limit(bytes, pos, module, max_type_idx)?;
            }
        ...
        0x5F => {
            ...
            for _ in 0..field_count {
                field_types.push(decode_storage_type_with_limit(
                    bytes,
                    pos,
                    module,
                    max_type_idx,
    ...
    for _i in 0..count {
        ...
        match kind_byte {
            ...
            0x02 => {
                ...
                let min_raw = if is_memory64 {
                    decode_leb128_u64(bytes, pos)?
                } else {
                    decode_leb128_u32(bytes, pos)? as u64
                };
...
fn decode_code_section(
    ...
    for i in 0..count {
        ...
        // INVARIANT: code bytes are stored contiguously in module.code. Each function's
        // code_offset is set to the current end of module.code, and code_len is the byte
        // count appended, so code_offset..code_offset+code_len is always a valid range.
        ...
        module
            .code
            .extend_from_slice(&bytes[*pos..*pos + code_bytes]);
...
fn decode_table_section(
    ...
    for _ in 0..count {
        ...
        if elemtype == 0x63 || elemtype == 0x64 {
            ...
            elemtype,
            0x6E | 0x6D | 0x6C | 0x6B | 0x6A | 0x73 | 0x72 | 0x71 | 0x74 | 0x69 | 0x68
        ) {
        ...
        let et = match elemtype {
            ...
            0x64 => {
                // (ref ht) = non-nullable
                if elem_heap_type == -16 {
                    ValType::TypedFuncRef
                } else if elem_heap_type == -17 {
                    ValType::ExternRef
                } else if elem_heap_type == -18 {
                    ValType::AnyRef
                } else if elem_heap_type == -19 {
                    ValType::EqRef
                } else if elem_heap_type == -20 {
                    ValType::I31Ref
                } else if elem_heap_type == -21 {
                    ValType::StructRef
                } else if elem_heap_type == -22 {
                    ValType::ArrayRef
                } else if elem_heap_type == -23 {
                    ValType::ExnRef
                } else if elem_heap_type == -15 {
                    ValType::NoneRef
                } else if elem_heap_type == -13 {
                    ValType::NullFuncRef
                } else if elem_heap_type == -14 {
                    ValType::NullExternRef
                } else if elem_heap_type == -12 {
                    ValType::ExnRef
                } else {
                    ValType::TypedFuncRef
                }
...
fn decode_global_section(
    ...
    for _ in 0..count {
        ...
        let saved_pos = *pos;
        let first_byte = if saved_pos < bytes.len() {
            bytes[saved_pos]
        } else {
            0
        };
        ...
        let val_type =
            decode_valtype_gc_aware_with_limit(bytes, pos, module, module.func_types.len() as u32)?;
        let mt = read_byte(bytes, pos)?;
        if mt > 1 {
            return Err(WasmError::InvalidSection);
        }
        ...
        module.globals.push(GlobalDef {
            ...
            init_global_ref,
            init_expr_type,
            init_expr_stack_depth,
            init_func_ref,
            init_expr_bytes,
            heap_type: global_heap_type,
            has_non_const,
        });
    }
    Ok(())
    ...
    for _ in 0..count {
        ...
        match flags {
            0 => {
                ...
                if num_elems > MAX_ELEMENTS_PER_SEGMENT {
                    return Err(WasmError::InvalidSection);
                }
            ...
            5 => {
                ...
                let elem_type =
                    decode_reftype_real_with_limit(bytes, pos, module.func_types.len() as u32)?;
                let num_elems = decode_leb128_u32(bytes, pos)? as usize;
                if num_elems > MAX_ELEMENTS_PER_SEGMENT {
                    return Err(WasmError::InvalidSection);
                }
