...
impl V128 {
    ...
    pub fn as_i16x8(self) -> [i16; 8] {
        let mut r = [0i16; 8];
        for i in 0..8 {
            r[i] = i16::from_le_bytes([self.0[i * 2], self.0[i * 2 + 1]]);
        }
        r
    }
    pub fn as_u16x8(self) -> [u16; 8] {
        let mut r = [0u16; 8];
        for i in 0..8 {
            r[i] = u16::from_le_bytes([self.0[i * 2], self.0[i * 2 + 1]]);
        }
        r
    }
    ...
    pub fn from_i8x16(v: [i8; 16]) -> Self {
        let mut b = [0u8; 16];
        for i in 0..16 {
            b[i] = v[i] as u8;
        }
    ...
    pub fn from_i64x2(v: [i64; 2]) -> Self {
        let mut b = [0u8; 16];
        for i in 0..2 {
            let x = v[i].to_le_bytes();
            b[i * 8..i * 8 + 8].copy_from_slice(&x);
        }
        V128(b)
    }
