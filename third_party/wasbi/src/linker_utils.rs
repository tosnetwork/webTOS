...
use crate::decoder::{FuncTypeDef, GcTypeDef, StorageType, WasmModule};
...
pub fn type_indices_equivalent(
    ...
    match (si_a, si_b) {
        (Some(a), Some(b)) => {
            ...
            for i in 0..a.rec_group_size {
                ...
                match (gc_a, gc_b) {
                    ...
                    (Some(ga), Some(gb)) => {
                        if core::mem::discriminant(ga) != core::mem::discriminant(gb) {
                            return false;
                        }
                    }
                    (None, None) => {}
                ...
                if let (Some(sa), Some(sb)) = (si_a_i, si_b_i) {
                    ...
                    match (sa.supertype, sb.supertype) {
                        (None, None) => {}
                        (Some(sp_a), Some(sp_b)) => {
                            let in_rg_a = sp_a >= a.rec_group_start
                                && sp_a < a.rec_group_start + a.rec_group_size;
                            let in_rg_b = sp_b >= b.rec_group_start
                                && sp_b < b.rec_group_start + b.rec_group_size;
                            if in_rg_a && in_rg_b {
                                // Both inside: compare relative positions
                                if (sp_a - a.rec_group_start) != (sp_b - b.rec_group_start) {
                                    return false;
                                }
                            } else if !in_rg_a && !in_rg_b {
                                // Both outside: recursively check equivalence
                                if !type_indices_equivalent(mod_a, sp_a, mod_b, sp_b) {
                                    return false;
                                }
                            } else {
                                return false;
        ...
        _ => {
            match (
                mod_a.func_types.get(type_idx_a as usize),
                mod_b.func_types.get(type_idx_b as usize),
            ) {
    ...
    match (a, b) {
        ...
        (StorageType::RefType(va, ai), StorageType::RefType(vb, bi)) => {
            ...
            if in_a && in_b {
                ...
                type_indices_equivalent(mod_a, *ai, mod_b, *bi)
            } else {
                false
            }
        }
...
pub fn global_types_compatible(
    ...
    let imp_is_concrete =
        matches!(import_byte, 0x63 | 0x64) && matches!(import_heap_type, Some(ht) if ht >= 0);
    let imp_nullable = import_byte != 0x64
        && import_val_type != ValType::TypedFuncRef
        && import_val_type != ValType::NonNullableFuncRef;

    // Classify export
    let exp_is_concrete = matches!(export_heap_type, Some(ht) if ht >= 0);
    let exp_nullable = matches!(
        export_val_type,
        ValType::FuncRef | ValType::NullableTypedFuncRef | ValType::ExternRef
    );
