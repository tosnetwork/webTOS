...
use crate::decoder::ImportKind;
use crate::decoder::{GcTypeDef, StorageType, WasmModule};
use crate::types::{ValType, WasmError};
...
pub fn is_ref_compatible(a: ValType, b: ValType) -> bool {
    ...
    if a_funcref_family && b_funcref_family {
        return true;
    }
    // externref family
    ...
    if a_externref_family && b_externref_family {
        return true;
    }
    // any hierarchy: NoneRef is compatible with any ref in the any hierarchy
    if (a == ValType::NoneRef || b == ValType::NoneRef) && (is_ref_type(a) && is_ref_type(b)) {
        return true;
    }
    false
}
            ...
            match (sub_gc, super_gc) {
                ...
                (GcTypeDef::Func, GcTypeDef::Func) => {
                    ...
                    if idx < module.func_types.len() && super_idx < module.func_types.len() {
                        ...
                        if sub_ft.param_count != super_ft.param_count
                            || sub_ft.result_count != super_ft.result_count
                        {
                            return Err(WasmError::TypeMismatch);
                        }
...
pub fn storage_type_eq(a: &StorageType, b: &StorageType) -> bool {
    match (a, b) {
        ...
        (StorageType::Val(va), StorageType::RefType(vb, _))
        | (StorageType::RefType(va, _), StorageType::Val(vb)) => va == vb,
        _ => false,
    }
}
    ...
    match (a, b) {
        ...
        (StorageType::RefType(va, ai), StorageType::RefType(vb, bi)) => {
            if ai == bi {
                ...
                val_is_subtype(*va, *vb)
            }
        }
        (StorageType::Val(va), StorageType::RefType(vb, _))
        | (StorageType::RefType(va, _), StorageType::Val(vb)) => val_is_subtype(*va, *vb),
        _ => false,
    }
}
...
pub fn validate_init_expr_for_segment(
    info: &crate::decoder::InitExprInfo,
...
pub fn val_is_subtype(src: ValType, dst: ValType) -> bool {
    ...
    match (src, dst) {
        ...
        (
            ValType::TypedFuncRef,
            ValType::NonNullableFuncRef | ValType::NullableTypedFuncRef | ValType::FuncRef,
        ) => true,
        ...
        (
            ValType::I31Ref,
            ValType::EqRef | ValType::AnyRef | ValType::NullableEqRef | ValType::NullableAnyRef,
        ) => true,
...
pub fn is_type_index_subtype(src: u32, dst: u32, module: &WasmModule) -> bool {
    ...
    if types_equivalent_in_module(module, src, dst) {
        return true;
    }
    let mut current = src;
    for _ in 0..100 {
...
fn types_equivalent_in_module_depth(module: &WasmModule, a: u32, b: u32, depth: u32) -> bool {
    ...
    let si_a = match module.sub_types.get(a as usize) {
        Some(s) => s,
        None => return false,
    };
    let si_b = match module.sub_types.get(b as usize) {
        Some(s) => s,
        None => return false,
    };
    if si_a.rec_group_size != si_b.rec_group_size {
        return false;
    }
    ...
    if si_a.rec_group_start == si_b.rec_group_start {
        return false;
    }
        ...
        (Some(sa), Some(sb)) => {
            ...
            match (sa.supertype, sb.supertype) {
                ...
                (Some(sp_a), Some(sp_b)) => {
                    ...
                    if in_a && in_b {
                        if (sp_a - rg_a) != (sp_b - rg_b) {
                            return false;
                        }
                    } else if !in_a && !in_b {
                        if sp_a != sp_b
                            && !types_equivalent_in_module_depth(module, sp_a, sp_b, depth + 1)
                        {
                            return false;
                        }
                    } else {
    ...
    match (gc_a, gc_b) {
        (Some(GcTypeDef::Func), Some(GcTypeDef::Func)) => {
            if let (Some(fa), Some(fb)) = (
                module.func_types.get(idx_a as usize),
                module.func_types.get(idx_b as usize),
            ) {
        ...
        (
            ...
            if ft_a.len() != ft_b.len() || fm_a != fm_b {
                return false;
            }
            for i in 0..ft_a.len() {
                if !storage_types_rec_eq(module, &ft_a[i], rg_a, &ft_b[i], rg_b, rg_size, depth) {
                    return false;
                }
            }
        }
        (
            Some(GcTypeDef::Array {
                elem_type: et_a,
                elem_mutable: em_a,
            }),
            Some(GcTypeDef::Array {
            ...
            if !storage_types_rec_eq(module, et_a, rg_a, et_b, rg_b, rg_size, depth) {
                return false;
            }
        }
        (None, None) => {
    ...
    match (a, b) {
        ...
        (StorageType::RefType(va, ai), StorageType::RefType(vb, bi)) => {
            ...
            if in_a && in_b {
                ...
                ai == bi || types_equivalent_in_module_depth(module, *ai, *bi, depth + 1)
            } else {
                false
            }
        }
