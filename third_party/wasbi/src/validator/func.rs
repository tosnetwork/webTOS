...
use crate::decoder::{FuncDef, FuncTypeDef, ImportKind, WasmModule};
#[cfg(feature = "gc")]
use crate::decoder::{GcTypeDef, StorageType};
use crate::types::{ValType, WasmError, MAX_LOCALS};
...
impl<'a> Validator<'a> {
    ...
    // INVARIANT: after an `unreachable` instruction, the control frame is marked
    // unreachable and the operand stack becomes polymorphic — pop_opd returns
    // StackType::Unknown instead of erroring, allowing any instruction sequence.
    fn pop_opd(&mut self) -> Result<StackType, WasmError> {
        ...
        self.opd_stack.pop().ok_or(WasmError::StackUnderflow)
    }
    ...
    fn read_i32(&mut self) -> Result<i32, WasmError> {
        crate::decoder::decode_leb128_i32(self.code, &mut self.pc)
    }

    fn read_i64(&mut self) -> Result<i64, WasmError> {
        crate::decoder::decode_leb128_i64(self.code, &mut self.pc)
    }

    fn read_u64(&mut self) -> Result<u64, WasmError> {
        crate::decoder::decode_leb128_u64(self.code, &mut self.pc)
    }
    ...
    fn read_memarg(&mut self, max_align: u32) -> Result<(), WasmError> {
        ...
        if self.module.multi_memory_enabled && mem_idx >= self.module.memory_count {
        ...
        if raw == -0x40 {
            ...
            let vt = match raw {
                ...
                -0x1D => {
                    ...
                    match heap_type {
                        ...
                        _ => {
                            ...
                            if heap_type >= 0
                                && (heap_type as u32) >= self.module.func_types.len() as u32
                            {
                                return Err(WasmError::TypeMismatch);
                            }
                ...
                -0x1C => {
                    ...
                    let heap_type = self.read_i32()?;
                    match heap_type {
                        -0x10 => ValType::NonNullableFuncRef, // func (non-nullable)
    ...
    /// Read a single value type from the code stream, handling multi-byte ref types.
    /// This is used by typed select and anywhere a valtype is encoded inline.
    fn read_valtype(&mut self) -> Result<ValType, WasmError> {
        let b = self.read_u32()? as u8;
        match b {
            0x7F => Ok(ValType::I32),
            0x7E => Ok(ValType::I64),
            0x7D => Ok(ValType::F32),
            ...
            0x72 => Ok(ValType::NullExternRef), // nullexternref
            0x73 => Ok(ValType::NullFuncRef),   // nullfuncref
            0x63 => {
                // (ref null ht) — read heap type
                let ht = self.read_i32()?;
                self.resolve_heap_type(ht, true)
            }
            0x64 => {
                // (ref ht) — read heap type
                let ht = self.read_i32()?;
                self.resolve_heap_type(ht, false)
            }
            _ => Err(WasmError::TypeMismatch),
        }
    }

    /// Resolve a heap type code to a ValType, validating type indices.
    fn resolve_heap_type(&self, heap_type: i32, nullable: bool) -> Result<ValType, WasmError> {
        match heap_type {
            -0x10 => Ok(if nullable {
                ValType::FuncRef
            } else {
            ...
            -0x0D => Ok(ValType::NullFuncRef),   // nofunc
            -0x0C => Ok(ValType::ExnRef),        // noexn
            _ => {
                // Concrete type index — validate it exists
                if heap_type < 0 {
                    return Err(WasmError::TypeMismatch);
                }
                let gc_len = self.module.gc_types.len();
                let ft_len = self.module.func_types.len();
                let max_types = if gc_len > 0 { gc_len } else { ft_len };
                if (heap_type as usize) >= max_types {
                    return Err(WasmError::TypeMismatch);
                }
                Ok(if nullable {
                    ValType::NullableTypedFuncRef
    ...
    fn func_type(&self, func_idx: u32) -> Result<&'a FuncTypeDef, WasmError> {
        let type_idx = if (func_idx as usize) < self.func_import_count {
            self.module
                .func_import_type(func_idx)
                .ok_or(WasmError::FunctionNotFound(func_idx))? as usize
        } else {
            ...
            match opcode {
                ...
                0x07 => {
                    ...
                    #[cfg(feature = "exceptions")]
                    {
                        ...
                        // catch is only valid inside a try block (0x06) or after catch/catch_all
                        if let Some(frame) = self.ctrl_stack.last() {
                            if !matches!(frame.opcode, 0x06 | 0x07 | 0x19) {
                ...
                0x0E => {
                    ...
                    if count > crate::types::MAX_BR_TABLE_SIZE {
                        return Err(WasmError::InvalidSection);
                    }
                ...
                0x11 => {
                    ...
                    // call_indirect requires funcref-family table
                ...
                0x13 => {
                    ...
                    // return_call_indirect requires funcref-family table
                    let tbl_et = table_elem_type(self.module, table_idx, self.table_import_count);
                    if !matches!(
                        tbl_et,
                        ValType::FuncRef
                ...
                0x18 => {
                    ...
                    #[cfg(feature = "exceptions")]
                    {
                        // delegate is only valid inside a try block (0x06) or catch block
                        if let Some(frame) = self.ctrl_stack.last() {
                            if !matches!(frame.opcode, 0x06 | 0x07 | 0x19) {
                                return Err(WasmError::InvalidSection);
                            }
                ...
                0x19 => {
                    ...
                    #[cfg(feature = "exceptions")]
                    {
                        // catch_all is only valid inside a try block (0x06) or after catch/catch_all
                        if let Some(frame) = self.ctrl_stack.last() {
                            if !matches!(frame.opcode, 0x06 | 0x07 | 0x19) {
                                return Err(WasmError::InvalidSection);
                            }
                ...
                0x3F => {
                    ...
                    let mem_idx = if self.module.multi_memory_enabled {
                        let idx = self.read_u32()?;
                        if idx >= self.module.memory_count {
                            return Err(WasmError::MemoryOutOfBounds);
                        }
                        idx
                    } else {
                ...
                0xD0 => {
                    ...
                    let vt = match heap_type {
                        ...
                        -0x0D => ValType::NullFuncRef,                        // nofunc
                        -0x0E => ValType::NullExternRef,                      // noextern
                        -0x0C => ValType::ExnRef,                             // noexn
                        _ if heap_type >= 0 => ValType::NullableTypedFuncRef, // concrete type
                        _ => ValType::NoneRef,
                    };
                    self.push_val(vt);
                }
                ...
                0xD3 => {
                    ...
                    if let StackType::Known(t) = ref1 {
                        if is_not_eq_subtype(t) {
                            return Err(WasmError::TypeMismatch);
                        }
                    }
                    if let StackType::Known(t) = ref2 {
                        if is_not_eq_subtype(t) {
                            return Err(WasmError::TypeMismatch);
                        }
                    }
                ...
                0xFC => {
                    ...
                    match sub {
                        ...
                        8 => {
                            ...
                            if !self.module.has_memory
                                || (if self.module.multi_memory_enabled {
                                    mem_idx >= self.module.memory_count
                                } else {
                                    mem_idx > 0
                                })
                            {
                        ...
                        10 => {
                            ...
                            if !self.module.has_memory
                                || (if self.module.multi_memory_enabled {
                                    dst >= self.module.memory_count
                                        || src >= self.module.memory_count
                                } else {
                                    dst > 0 || src > 0
                                })
                            {
                            ...
                            let size_type = if dst_at == ValType::I64 || src_at == ValType::I64 {
                                ValType::I64
                            } else {
                                ValType::I32
                            };
                        ...
                        12 => {
                            ...
                            let tbl_et =
                                table_elem_type(self.module, tbl_idx, self.table_import_count);
                        ...
                        14 => {
                            ...
                            if dst_idx as usize >= self.total_tables
                                || src_idx as usize >= self.total_tables
                            {
                                return Err(WasmError::TableIndexOutOfBounds);
                            }
                            // Check element type compatibility: src must be subtype of dst
                            let dst_et =
                                table_elem_type(self.module, dst_idx, self.table_import_count);
                            let src_et =
                                table_elem_type(self.module, src_idx, self.table_import_count);
                            ...
                            let n_type = if src_it == ValType::I32 || dst_it == ValType::I32 {
                                ValType::I32
                            } else {
                                ValType::I64
                            };
                ...
                0xFB => {
                    ...
                    #[cfg(feature = "gc")]
                    {
                        ...
                        match sub {
                            0 => {
                                // struct.new: typeidx — pop N fields, push structref
                                let type_idx = self.read_u32()?;
                                // Pop field values in reverse order
                                if let Some(GcTypeDef::Struct { field_types, .. }) =
                                    self.module.gc_types.get(type_idx as usize)
                                {
                            ...
                            1 => {
                                // struct.new_default: typeidx — push structref
                                let _type_idx = self.read_u32()?;
                                self.push_opd(StackType::Unknown);
                            }
                            2..=4 => {
                                // struct.get/get_s/get_u: typeidx fieldidx — pop ref, push val
                                let type_idx = self.read_u32()?;
                                let field_idx = self.read_u32()? as usize;
                                let _ = self.pop_opd()?;
                                // Determine the field type from the GC type definition
                                let field_vt = if let Some(GcTypeDef::Struct {
                                    ref field_types,
                                    ..
                                }) = self.module.gc_types.get(type_idx as usize)
                                {
                                    if let Some(st) = field_types.get(field_idx) {
                                        // struct.get_s/get_u (3/4) are for packed types only
                                        // struct.get (2) returns the unpacked type
                                        st.unpack()
                                    } else {
                                        return Err(WasmError::TypeMismatch);
                                    }
                                } else {
                                    // Unknown type — push Unknown
                                    self.push_opd(StackType::Unknown);
                                    continue;
                                };
                                self.push_val(field_vt);
                            }
                            5 => {
                                // struct.set: typeidx fieldidx — pop ref, pop val
                                let type_idx = self.read_u32()?;
                                let field_idx = self.read_u32()? as usize;
                                // Check field is mutable
                                if let Some(GcTypeDef::Struct { field_muts, .. }) =
                                    self.module.gc_types.get(type_idx as usize)
                                {
                            ...
                            6 => {
                                // array.new: typeidx — pop init, pop len, push arrayref
                                let _ = self.read_u32()?;
                            ...
                            7 => {
                                // array.new_default: typeidx — pop len, push arrayref
                                let _ = self.read_u32()?;
                                let _ = self.pop_opd()?; // len
                                self.push_opd(StackType::Unknown);
                            }
                            ...
                            9 | 10 => {
                                // array.new_data/elem: typeidx + idx — pop offset, pop len, push ref
                                let _ = self.read_u32()?;
                                let _ = self.read_u32()?;
                                let _ = self.pop_opd()?; // len
                            ...
                            11..=13 => {
                                // array.get/get_s/get_u: typeidx — pop idx, pop ref, push val
                                let _ = self.read_u32()?;
                            ...
                            14 => {
                                // array.set: typeidx — pop val, pop idx, pop ref
                                let type_idx = self.read_u32()?;
                                // Check array is mutable
                                if let Some(GcTypeDef::Array { elem_mutable, .. }) =
                                    self.module.gc_types.get(type_idx as usize)
                                {
                                    if !elem_mutable {
                                        return Err(WasmError::TypeMismatch);
                            ...
                            15 => {
                                // array.len: pop ref, push i32
                                let _ = self.pop_opd()?;
                                self.push_val(ValType::I32);
                            }
                            16 => {
                                // array.fill: typeidx — pop len, pop val, pop idx, pop ref
                                let type_idx = self.read_u32()?;
                                // Check array is mutable and validate fill value type
                                if let Some(GcTypeDef::Array {
                                    elem_mutable,
                                    ref elem_type,
                                }) = self.module.gc_types.get(type_idx as usize)
                                {
                            ...
                            17 => {
                                // array.copy: dst_typeidx + src_typeidx — pop len, pop src_idx, pop src_ref, pop dst_idx, pop dst_ref
                                let dst_type = self.read_u32()?;
                                let src_type = self.read_u32()?;
                                // Check destination array is mutable
                                if let Some(GcTypeDef::Array { elem_mutable, .. }) =
                                    self.module.gc_types.get(dst_type as usize)
                                {
                                    if !elem_mutable {
                                        return Err(WasmError::TypeMismatch);
                                    }
                                ...
                                {
                                    let dst_elem =
                                        if let Some(GcTypeDef::Array { ref elem_type, .. }) =
                                            self.module.gc_types.get(dst_type as usize)
                                        {
                                            Some(*elem_type)
                                        } else {
                                    ...
                                    let src_elem =
                                        if let Some(GcTypeDef::Array { ref elem_type, .. }) =
                                            self.module.gc_types.get(src_type as usize)
                                        {
                                            Some(*elem_type)
                                        } else {
                                            None
                                    ...
                                    if let (Some(dst_st), Some(src_st)) = (dst_elem, src_elem) {
                                        ...
                                        let compatible = match (&src_st, &dst_st) {
                                            ...
                                            (StorageType::Val(sv), StorageType::Val(dv)) => {
                                                val_is_subtype(*sv, *dv)
                                            }
                                            // RefType with same type index is compatible
                                            (
                                                StorageType::RefType(_, si),
                                                StorageType::RefType(_, di),
                                            ) => {
                                                if si == di {
                                                    true
                                                } else {
                                                    // Check subtype chain: src must be a declared subtype of dst
                                                    is_type_index_subtype(*si, *di, self.module)
                                                }
                                            }
                                            // RefType vs Val: check abstract subtyping only
                                            (StorageType::RefType(sv, _), StorageType::Val(dv)) => {
                                                val_is_subtype(*sv, *dv)
                                            }
                                            (StorageType::Val(sv), StorageType::RefType(dv, _)) => {
                                                val_is_subtype(*sv, *dv)
                                            }
                                            _ => false,
                                        };
                            ...
                            18 => {
                                // array.init_data: typeidx + dataidx — pop len, pop src_off, pop dst_idx, pop ref
                                let type_idx = self.read_u32()?;
                                let _ = self.read_u32()?;
                            ...
                            19 => {
                                // array.init_elem: typeidx + elemidx — pop len, pop src_off, pop dst_idx, pop ref
                                let type_idx = self.read_u32()?;
                                ...
                                }) = self.module.gc_types.get(type_idx as usize)
                                {
                                    ...
                                    if let Some(seg) =
                                        self.module.element_segments.get(elem_idx as usize)
                                    {
                                        ...
                                        if is_ref_type(arr_unpacked)
                                            && is_ref_type(seg_type)
                                            && !val_is_subtype(seg_type, arr_unpacked)
                                        {
                                            return Err(WasmError::TypeMismatch);
                                        }
                                    }
                            ...
                            20 | 21 => {
                                // ref.test: heaptype — pop ref, push i32
                                let _ = self.read_i32()?;
                                let _ = self.pop_opd()?;
                                self.push_val(ValType::I32);
                            }
                            22 | 23 => {
                                // ref.cast: heaptype — pop ref, push ref
                                let _ = self.read_i32()?;
                                let _ = self.pop_opd()?;
                                self.push_opd(StackType::Unknown);
                            }
                            24 | 25 => {
                                ...
                                fn heap_type_to_val(ht: i32, nullable: bool) -> Option<ValType> {
                                    match ht {
                                        ...
                                        -21 => {
                                            if nullable {
                                                Some(ValType::NullableStructRef)
                                            } else {
                                                Some(ValType::StructRef)
                                            }
                                        }
                                        -22 => {
                                            if nullable {
                                                Some(ValType::NullableArrayRef)
                                            } else {
                                                Some(ValType::ArrayRef)
                                            }
                                        }
                                        -16 => {
                                            if nullable {
                                                ...
                                                Some(ValType::NonNullableFuncRef)
                                            }
                                        }
                                        -17 => Some(ValType::ExternRef), // extern
                                        -15 => Some(ValType::NoneRef),   // none
                                        -13 => Some(ValType::NullFuncRef), // nofunc
                                        -14 => Some(ValType::NullExternRef), // noextern
                                        -23 => Some(ValType::ExnRef),    // exn
                                        _ if ht >= 0 => {
                                            if nullable {
                                                Some(ValType::NullableTypedFuncRef)
                                            } else {
                                                Some(ValType::TypedFuncRef)
                                            }
                                        }
                                        _ => None,
                                    }
                                ...
                                // Ref type subtyping: (ref null2? ht2) <: (ref null1? ht1) requires:
                                // 1. If ht2 is nullable, ht1 must also be nullable
                                if ht2_null && !ht1_null {
                                    return Err(WasmError::TypeMismatch);
                                }
                                if let (Some(t1), Some(t2)) = (
                                    heap_type_to_val(ht1, ht1_null),
                                    heap_type_to_val(ht2, ht2_null),
                                ) {
                                    // 2. ht2 must be a subtype of ht1 (heap type subtyping)
                                ...
                                let branch_top = if sub == 24 {
                                    // br_on_cast: branch sends rt2
                                    heap_type_to_val(ht2, ht2_null)
                                } else {
                                    // br_on_cast_fail: branch sends rt1 \ rt2 (difference)
                                    heap_type_to_val(ht1, diff_null)
                                };
                                let fallthru_top = if sub == 24 {
                                    // br_on_cast: fall-through gets rt1 \ rt2
                                    heap_type_to_val(ht1, diff_null)
                                } else {
                                    // br_on_cast_fail: fall-through gets rt2
                                    heap_type_to_val(ht2, ht2_null)
                                };
                            ...
                            26 | 27 => {
                                // any.convert_extern, extern.convert_any: pop ref, push ref
                                let _ = self.pop_opd()?;
                                self.push_opd(StackType::Unknown);
                            }
                            28 => {
...
pub fn validate_function_body(
    ...
    let safe_local_count = (func.local_count as usize).min(MAX_LOCALS);
    for i in 0..safe_local_count {
...
fn read_leb128_u32(code: &[u8], pc: &mut usize) -> Result<u32, WasmError> {
    crate::decoder::decode_leb128_u32(code, pc)
}
