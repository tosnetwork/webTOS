...
mod simd;
#[path = "runtime/stores.rs"]
mod stores;

use crate::config::Config;
use crate::decoder::{ImportKind, WasmModule};
use crate::types::*;
use alloc::vec;
use alloc::vec::Vec;
use stores::*;
...
pub struct WasmInstance {
    ...
    pub(crate) gc_store: GcStore,
    pub(crate) fuel_state: FuelState,
    ...
    block_depth: usize,
    pub(crate) max_memory_pages: usize,
    ...
    pub(crate) legacy_exception_store: Vec<Option<Vec<Value>>>,
...
impl WasmInstance {
    ...
    pub fn new(module: WasmModule, fuel: u64) -> Result<Self, WasmError> {
        Self::with_limits(
            ...
            MAX_MEMORY_PAGES,
            MAX_TABLE_SIZE,
            crate::fuel::FuelCosts::default(),
        )
    }

    /// Create a new instance with a specific runtime class.
    // INVARIANT: initialization order matters — memories are created first, then globals
    // (which may reference memory via data segments), then tables, then element segments
    // are applied (which may reference both tables and functions).
    pub fn with_class(
        module: WasmModule,
        fuel: u64,
    ...
    /// Create a new instance using the embedder's full engine configuration.
    pub fn with_config(module: WasmModule, config: &Config) -> Result<Self, WasmError> {
        Self::with_limits(
            module,
            config.fuel,
    ...
    fn with_limits(
        ...
        fuel_costs: crate::fuel::FuelCosts,
    ) -> Result<Self, WasmError> {
        ...
        for t in &module.tables {
            let default_entry = if let Some(ref expr_bytes) = t.init_expr_bytes {
                let mut expr_pos = 0;
                let init_val = crate::decoder::eval_init_expr_with_globals(
                    expr_bytes,
                    &mut expr_pos,
                    &globals,
                );
        ...
        for (seg_idx, seg) in module.element_segments.iter().enumerate() {
            ...
            let is_func_table = tbl_idx < module.tables.len()
                && matches!(
        ...
        let mut inst = WasmInstance {
            module,
            stack: vec![Value::I32(0); max_stack_height],
            stack_ptr: 0,
            max_stack_height,
            ...
            gc_store: GcStore::default(),
            fuel_state: FuelState::new(fuel, fuel_costs),
            pc: 0,
        ...
        for seg in &inst.module.data_segments {
            ...
            if mem_idx >= inst.memory_store.memories.len() {
            ...
            if dst_start.saturating_add(len) > inst.memory_store.memory_sizes[mem_idx] {
                return Err(WasmError::MemoryOutOfBounds);
            }

            if src_start.saturating_add(len) <= inst.module.code.len() {
                inst.memory_store.memories[mem_idx][dst_start..dst_start + len]
        ...
        for (i, seg) in inst.module.element_segments.iter().enumerate() {
            if seg.mode == ElemMode::Active || seg.mode == ElemMode::Declarative {
                inst.table_store.dropped_elems[i] = true;
            }
        }
    ...
    // INVARIANT: stack_ptr <= stack.len() is enforced by push (bounds check before write),
    // and stack_ptr > 0 is enforced by pop (underflow check before decrement).
    ...
    fn read_leb128_u32(&mut self) -> Result<u32, WasmError> {
        crate::decoder::decode_leb128_u32(&self.module.code, &mut self.pc)
    }

    fn read_leb128_i32(&mut self) -> Result<i32, WasmError> {
        crate::decoder::decode_leb128_i32(&self.module.code, &mut self.pc)
    }

    fn read_leb128_i64(&mut self) -> Result<i64, WasmError> {
        crate::decoder::decode_leb128_i64(&self.module.code, &mut self.pc)
    }
    ...
    fn tbl(&self, idx: usize) -> usize {
        resolve_alias(&self.table_store.table_aliases, idx)
    }

    // INVARIANT: mem/mem_mut resolve aliases before indexing, so aliased memories
    // (from imports) transparently redirect to the canonical memory. Falls back to
    // memory 0 if the resolved index is out of range (defensive, not reachable after validation).
    /// Get memory slice reference by index, defaulting to memory 0.
    #[inline]
    fn mem(&self, idx: usize) -> &Vec<u8> {
        let ri = resolve_alias(&self.memory_store.memory_aliases, idx);
        if ri < self.memory_store.memories.len() {
            &self.memory_store.memories[ri]
        } else {
            &self.memory_store.memories[0]
        }
    }
    ...
    fn mem_mut(&mut self, idx: usize) -> &mut Vec<u8> {
        let ri = resolve_alias(&self.memory_store.memory_aliases, idx);
        if ri < self.memory_store.memories.len() {
            &mut self.memory_store.memories[ri]
        } else {
            &mut self.memory_store.memories[0]
        }
    }
    ...
    fn mem_size(&self, idx: usize) -> usize {
        let ri = resolve_alias(&self.memory_store.memory_aliases, idx);
        if ri < self.memory_store.memory_sizes.len() {
            self.memory_store.memory_sizes[ri]
        } else {
            0
        }
    }
    ...
    fn enter_function(&mut self, func_idx: u32, keep_args_on_stack: bool) -> Result<(), WasmError> {
        let local_func_idx = (func_idx as usize)
            .checked_sub(self.module.func_import_count())
    ...
    /// Mutably borrow the decoded module.
    ...
    pub fn get_fuel(&self) -> u64 {
        self.fuel_state.fuel
    }
    /// Set the fuel budget.
    pub fn set_fuel(&mut self, fuel: u64) {
        self.fuel_state.fuel = fuel;
    }

    /// Check whether execution has finished.
    pub fn is_finished(&self) -> bool {
        self.fuel_state.finished
    }
    /// Mark execution as finished (or not).
    pub fn set_finished(&mut self, finished: bool) {
        self.fuel_state.finished = finished;
    }

    /// Number of linear memories.
    pub fn memory_count(&self) -> usize {
        self.memory_store.memories.len()
    }
    /// Size of a linear memory in bytes.
    pub fn get_memory_size(&self, idx: usize) -> Option<usize> {
        self.memory_store.memory_sizes.get(idx).copied()
    }
    /// Set the tracked size of a linear memory.
    pub fn set_memory_size(&mut self, idx: usize, size: usize) {
        if let Some(s) = self.memory_store.memory_sizes.get_mut(idx) {
            *s = size;
        }
    }
    /// Borrow a linear memory by index.
    pub fn get_memory(&self, idx: usize) -> Option<&Vec<u8>> {
        self.memory_store.memories.get(idx)
    }
    /// Mutably borrow a linear memory by index.
    pub fn get_memory_mut(&mut self, idx: usize) -> Option<&mut Vec<u8>> {
        self.memory_store.memories.get_mut(idx)
    }
    /// Direct access to the memories vector.
    pub fn memories(&self) -> &[Vec<u8>] {
        &self.memory_store.memories
    }
    /// Mutable access to the memories vector.
    pub fn memories_mut(&mut self) -> &mut Vec<Vec<u8>> {
        &mut self.memory_store.memories
    }
    /// Direct access to the memory_sizes vector.
    pub fn memory_sizes(&self) -> &[usize] {
        &self.memory_store.memory_sizes
    }
    /// Mutable access to the memory_sizes vector.
    pub fn memory_sizes_mut(&mut self) -> &mut Vec<usize> {
        &mut self.memory_store.memory_sizes
    }
    /// Memory alias array.
    pub fn memory_aliases(&self) -> &[Option<usize>] {
        &self.memory_store.memory_aliases
    }
    /// Mutable memory alias array.
    pub fn memory_aliases_mut(&mut self) -> &mut Vec<Option<usize>> {
        &mut self.memory_store.memory_aliases
    }

    /// Number of globals.
    pub fn global_count(&self) -> usize {
        self.global_store.globals.len()
    }
    /// Read a global value by index.
    pub fn get_global(&self, idx: usize) -> Option<Value> {
        self.global_store.globals.get(idx).copied()
    }
    /// Write a global value by index.
    pub fn set_global(&mut self, idx: usize, val: Value) {
        if let Some(g) = self.global_store.globals.get_mut(idx) {
            *g = val;
        }
    }
    /// Direct access to globals vector.
    pub fn globals(&self) -> &[Value] {
        &self.global_store.globals
    }
    /// Mutable access to globals vector.
    pub fn globals_mut(&mut self) -> &mut Vec<Value> {
        &mut self.global_store.globals
    }
    /// Global alias array.
    pub fn global_aliases(&self) -> &[Option<usize>] {
        &self.global_store.global_aliases
    }
    /// Mutable global alias array.
    pub fn global_aliases_mut(&mut self) -> &mut Vec<Option<usize>> {
        &mut self.global_store.global_aliases
    }

    /// Number of tables.
    pub fn table_count(&self) -> usize {
        self.table_store.tables.len()
    }
    /// Borrow a table by index.
    pub fn get_table(&self, idx: usize) -> Option<&Vec<Option<u32>>> {
        self.table_store.tables.get(idx)
    }
    /// Mutably borrow a table by index.
    pub fn get_table_mut(&mut self, idx: usize) -> Option<&mut Vec<Option<u32>>> {
        self.table_store.tables.get_mut(idx)
    }
    /// Direct access to tables vector.
    pub fn tables(&self) -> &[Vec<Option<u32>>] {
        &self.table_store.tables
    }
    /// Mutable access to tables vector.
    pub fn tables_mut(&mut self) -> &mut Vec<Vec<Option<u32>>> {
        &mut self.table_store.tables
    }
    /// Table alias array.
    pub fn table_aliases(&self) -> &[Option<usize>] {
        &self.table_store.table_aliases
    }
    /// Mutable table alias array.
    pub fn table_aliases_mut(&mut self) -> &mut Vec<Option<usize>> {
        &mut self.table_store.table_aliases
    }

    /// Borrow the GC heap.
    pub fn gc_heap(&self) -> &[GcObject] {
        &self.gc_store.gc_heap
    }
    /// Mutable access to the GC heap.
    pub fn gc_heap_mut(&mut self) -> &mut Vec<GcObject> {
        &mut self.gc_store.gc_heap
    }

    /// Borrow the operand stack up to the stack pointer.
    pub fn stack_slice(&self) -> &[Value] {
        &self.stack[..self.stack_ptr]
    }
    /// Reset the stack pointer to zero.
    pub fn reset_stack_ptr(&mut self) {
        self.stack_ptr = 0;
    }
    ...
    pub fn set_stack_ptr(&mut self, ptr: usize) {
        self.stack_ptr = ptr;
    }

    /// Dropped element segments tracker.
    pub fn dropped_elems(&self) -> &[bool] {
        &self.table_store.dropped_elems
    }
    /// Mutable access to dropped element segments.
    pub fn dropped_elems_mut(&mut self) -> &mut Vec<bool> {
        &mut self.table_store.dropped_elems
    }
    /// Dropped data segments tracker.
    pub fn dropped_data(&self) -> &[bool] {
        &self.dropped_data
    }

    /// Element GC values (re-evaluated expression-based elements).
    pub fn elem_gc_values(&self) -> &[Vec<Value>] {
        &self.gc_store.elem_gc_values
    }
    /// Mutable access to element GC values.
    pub fn elem_gc_values_mut(&mut self) -> &mut Vec<Vec<Value>> {
        &mut self.gc_store.elem_gc_values
    }
    ...
    // INVARIANT: func_idx is an absolute index across the combined import+local function
    // index space. Indices below func_import_count() are imports (handled via HostCall);
    // indices at or above that threshold are local module functions.
    pub fn call_func(&mut self, func_idx: u32, args: &[Value]) -> ExecResult {
        // Reset execution state for new call (important after traps)
        self.fuel_state.finished = false;
        self.call_depth = 0;
    ...
    // INVARIANT: fuel is decremented before executing each instruction, not after.
    // This ensures OutOfFuel is returned before any work is done.
    // Base cost is charged here; bulk/call/simd operations charge extra via consume_extra().
    fn step(&mut self) -> ExecResult {
        if !self.fuel_state.consume() {
        ...
        match opcode {
            ...
            0x04 => {
                ...
                let has_else = else_or_end_pc > 0 && self.module.code[else_or_end_pc - 1] == 0x05;
            ...
            0x05 => {
                ...
                let base05 = if self.call_depth > 0 {
                    self.call_stack[self.call_depth - 1].block_stack_base
                } else {
                    0
                };
            ...
            0x0B => {
                // end
                let base0b = if self.call_depth > 0 {
                    self.call_stack[self.call_depth - 1].block_stack_base
                } else {
                    0
                };
            ...
            0x06 => {
                ...
                let block_type = try_exec!(self.read_block_type());
                let (param_count, result_count) = self.decode_block_type(block_type);
                let (legacy_catches, legacy_catch_count, end_pc, delegate_label) =
                    try_exec!(self.scan_legacy_try());
                let stack_base = self.stack_ptr - param_count as usize;
            ...
            0x07 => {
                ...
                let base07 = if self.call_depth > 0 {
                    self.call_stack[self.call_depth - 1].block_stack_base
                } else {
                    0
                };
            ...
            0x19 => {
                ...
                let base19 = if self.call_depth > 0 {
                    self.call_stack[self.call_depth - 1].block_stack_base
                } else {
                    0
                };
            ...
            0x10 => {
                // call — extra fuel for function invocation overhead
                if !self
                    .fuel_state
                    .consume_extra(self.fuel_state.fuel_costs.call_extra())
                {
                    return ExecResult::OutOfFuel;
                }
            ...
            0x13 => {
                ...
                let tbl = if table_idx < self.table_store.tables.len() {
                    &self.table_store.tables[self.tbl(table_idx)]
                } else {
                ...
                let func_idx = match tbl[elem_idx] {
                    Some(idx) => idx,
                    None => {
                        return ExecResult::Trap(WasmError::UninitializedElement(elem_idx as u32))
                    }
                };
                ...
                let actual_type_idx = if (func_idx as usize) < self.module.func_import_count() {
                    ...
                    let local_idx =
                        match (func_idx as usize).checked_sub(self.module.func_import_count()) {
                            Some(i) => i,
            ...
            0x11 => {
                ...
                if tbl >= self.table_store.tables.len()
                    || elem_idx >= self.table_store.tables[self.tbl(tbl)].len()
                {
                    return ExecResult::Trap(WasmError::UndefinedElement);
                }
                let func_idx = match self.table_store.tables[self.tbl(tbl)][elem_idx] {
                    Some(idx) => idx,
                    None => {
                        return ExecResult::Trap(WasmError::UninitializedElement(elem_idx as u32))
                    }
            ...
            0x23 => {
                ...
                let idx = resolve_alias(&self.global_store.global_aliases, raw_idx);
                if idx >= self.global_store.globals.len() {
                    return ExecResult::Trap(WasmError::GlobalIndexOutOfBounds);
                }
                try_exec!(self.push(self.global_store.globals[idx]));
            }
            0x24 => {
                // global.set
                let raw_idx = try_exec!(self.read_leb128_u32()) as usize;
                ...
                let val = try_exec!(self.pop());
                self.global_store.globals[idx] = val;
            }

            0x25 => {
                // table.get
                let table_idx = try_exec!(self.read_leb128_u32()) as usize;
                let is_t64 = table_idx < self.module.tables.len()
                    && self.module.tables[table_idx].is_table64;
                let idx = if is_t64 {
                    try_exec!(self.pop_i64()) as usize
                } else {
                    try_exec!(self.pop_i32()) as usize
                };
                if table_idx >= self.table_store.tables.len() {
                    return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
                }
                if idx >= self.table_store.tables[self.tbl(table_idx)].len() {
                    return ExecResult::Trap(WasmError::TableIndexOutOfBounds);
                }
                let val = match self.table_store.tables[self.tbl(table_idx)][idx] {
                    None => Value::NullRef,
            ...
            0x26 => {
                ...
                let rt = resolve_alias(&self.table_store.table_aliases, table_idx);
                self.table_store.tables[rt][idx] = entry;
            }

            // ── Memory (0x28-0x40) (see runtime/memory.rs) ───
            0x28..=0x40 => {
                return memory::execute_memory(self, opcode);
            }
            ...
            0x43 => {
                // f32.const
                if self.runtime_class == RuntimeClass::ProofGrade {
                    return ExecResult::Trap(WasmError::FloatsDisabled);
                }
            ...
            0x45..=0xC4 => {
                return numeric::execute_numeric(self, opcode);
            }

            // ── 0xFC prefix: sat trunc + bulk memory + table ops (see runtime/fc_ops.rs) ─
            0xFC => {
                return fc_ops::execute_fc(self);
            }
            ...
            0xFD => {
                if !self
                    .fuel_state
                    .consume_extra(self.fuel_state.fuel_costs.simd_extra())
                {
                    return ExecResult::OutOfFuel;
                }
                return simd::execute_simd(self);
            }
            ...
            #[cfg(feature = "threads")]
            0xFE => {
                return atomic::execute_atomic(self);
            }
...
fn sat_trunc_f32_u32(v: f32) -> u32 {
    ...
    if v.is_infinite() {
        return if v.is_sign_positive() { u32::MAX } else { 0 };
    }
    if v >= 4294967296.0_f32 {
        return u32::MAX;
    }
    if v <= -1.0_f32 {
        return 0;
...
fn sat_trunc_f64_i32(v: f64) -> i32 {
    ...
    if v >= 2147483648.0_f64 {
        return i32::MAX;
    }
    if v <= -2147483649.0_f64 {
        return i32::MIN;
    }
    v as i32
}
...
fn sat_trunc_f32_i64(v: f32) -> i64 {
    ...
    if v <= -9223373136366403584.0_f32 {
        return i64::MIN;
    }
    v as i64
}
fn sat_trunc_f32_u64(v: f32) -> u64 {
    ...
    if v.is_infinite() {
        return if v.is_sign_positive() { u64::MAX } else { 0 };
    }
    if v >= 18446744073709551616.0_f32 {
        return u64::MAX;
    }
    if v <= -1.0_f32 {
        return 0;
...
fn sat_trunc_f64_i64(v: f64) -> i64 {
    ...
    if v >= 9223372036854775808.0_f64 {
        return i64::MAX;
    }
    if v <= -9223372036854777856.0_f64 {
        return i64::MIN;
    }
    v as i64
}
fn sat_trunc_f64_u64(v: f64) -> u64 {
    ...
    if v >= 18446744073709551616.0_f64 {
        return u64::MAX;
    }
    if v <= -1.0_f64 {
        return 0;
