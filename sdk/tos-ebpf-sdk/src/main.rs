//! `tos-ebpf` — TOS eBPF-lite SDK command-line tool.
//!
//! # Usage
//!
//! ```
//! tos-ebpf compile <input.ebpf> [-o <output.bin>]
//! tos-ebpf verify  <input.bin>
//! tos-ebpf disasm  <input.bin>
//! ```
//!
//! ## compile
...
fn cmd_disasm(args: &[String]) {
    ...
    println!(
        "; TOS eBPF-lite disassembly of '{}' ({} instruction{})",
        path.display(),
        insns.len(),
        if insns.len() == 1 { "" } else { "s" }
    );
...
fn usage() {
    eprintln!(
        "TOS eBPF-lite SDK v{}\n\
         \n\
         USAGE:\n\
         \ttos-ebpf compile <input.ebpf> [-o output.bin]   Assemble + verify\n\
         \ttos-ebpf verify  <input.bin>                    Verify binary\n\
         \ttos-ebpf disasm  <input.bin>                    Disassemble binary",
