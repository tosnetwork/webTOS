...
pub fn send(target_mailbox: u16, payload: &[u8]) -> AosResult<()> {
    let ret = unsafe {
...
pub fn send_blocking(target_mailbox: u16, payload: &[u8]) -> AosResult<()> {
    let ret = unsafe {
...
pub fn recv(mailbox_id: u16, buf: &mut [u8]) -> AosResult<usize> {
    let ret = unsafe {
...
pub fn recv_nonblocking(mailbox_id: u16, buf: &mut [u8]) -> AosResult<usize> {
    let ret = unsafe {
...
pub fn recv_timeout(mailbox_id: u16, buf: &mut [u8], timeout_ticks: u64) -> AosResult<usize> {
    let ret = unsafe {
...
pub fn destroy(mailbox_id: u16) -> AosResult<()> {
