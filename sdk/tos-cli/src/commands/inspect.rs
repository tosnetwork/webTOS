//! tos inspect — parse TOS serial event log
//!
//! Usage:
//!   tos inspect <event-log.txt>
//!
//! Reads a text file captured from TOS serial output and provides
...
pub fn run(args: &[String]) {
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("Usage: tos inspect <event-log.txt>");
        println!();
        println!("Parse TOS serial output and display statistics.");
        return;
    }
    ...
    let content = match fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[tos-inspect] Failed to read {}: {}", path, e);
            std::process::exit(1);
        }
    };
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║         TOS EVENT LOG ANALYSIS              ║");
    println!("╠══════════════════════════════════════════════╣");
