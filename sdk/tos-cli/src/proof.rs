...
//!
//! Reads an TOS execution proof file (ATSP format) and verifies
...
/// TOS Execution Proof (mirrors kernel's ExecutionProof)
...
pub fn verify_file(path: &str) {
    let data = match fs::read(path) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("[tos-verify] Failed to read {}: {}", path, e);
            std::process::exit(1);
        }
    };

    if data.len() < PROOF_SIZE {
        eprintln!("[tos-verify] File too small: {} bytes (need {})", data.len(), PROOF_SIZE);
        std::process::exit(1);
    }

    if &data[0..4] != PROOF_MAGIC {
        eprintln!("[tos-verify] Invalid magic: expected ATSP, got {:?}", &data[0..4]);
        std::process::exit(1);
    }

    if data[4] != PROOF_VERSION {
        eprintln!("[tos-verify] Unsupported version: {}", data[4]);
        std::process::exit(1);
    }
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║       TOS EXECUTION PROOF VERIFICATION      ║");
    println!("╠══════════════════════════════════════════════╣");
