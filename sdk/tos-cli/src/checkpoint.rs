...
//!
//! Parses TOS checkpoint binary files (raw disk images).
...
pub fn parse_and_display(data: &[u8]) {
    ...
    if header_data.len() < SECTOR_SIZE {
        eprintln!("[tos-replay] Data too small for checkpoint header");
        return;
    }
    ...
    if magic != CHECKPOINT_MAGIC {
        eprintln!("[tos-replay] Invalid checkpoint magic: {:#x} (expected ATSC = {:#x})", magic, CHECKPOINT_MAGIC);
        eprintln!("[tos-replay] Tip: use a raw disk image from 'make test' (e.g., /tmp/tos_test.img)");
        return;
    }
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║       TOS CHECKPOINT ANALYSIS               ║");
    println!("╠══════════════════════════════════════════════╣");
