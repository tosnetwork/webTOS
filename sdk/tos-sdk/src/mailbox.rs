...
use crate::syscall::*;
use crate::{TosResult, check};

/// Send a message to a target mailbox (non-blocking).
pub fn send(target_mailbox: u16, payload: &[u8]) -> TosResult<()> {
    let ret = unsafe {
...
pub fn send_blocking(target_mailbox: u16, payload: &[u8]) -> TosResult<()> {
    let ret = unsafe {
...
pub fn recv(mailbox_id: u16, buf: &mut [u8]) -> TosResult<usize> {
    let ret = unsafe {
...
pub fn recv_nonblocking(mailbox_id: u16, buf: &mut [u8]) -> TosResult<usize> {
    let ret = unsafe {
...
pub fn recv_timeout(mailbox_id: u16, buf: &mut [u8], timeout_ticks: u64) -> TosResult<usize> {
    let ret = unsafe {
...
pub fn destroy(mailbox_id: u16) -> TosResult<()> {
