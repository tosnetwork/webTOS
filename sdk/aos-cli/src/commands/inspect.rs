//! aos inspect — parse AOS serial event log
//!
//! Usage:
//!   aos inspect <event-log.txt>
//!
//! Reads a text file captured from AOS serial output and provides
...
pub fn run(args: &[String]) {
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("Usage: aos inspect <event-log.txt>");
        println!();
        println!("Parse AOS serial output and display statistics.");
        return;
    }
    ...
    let content = match fs::read_to_string(path) {
        Ok(c) => c,
        Err(e) => {
            eprintln!("[aos-inspect] Failed to read {}: {}", path, e);
            std::process::exit(1);
        }
    };
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║         AOS EVENT LOG ANALYSIS              ║");
    println!("╠══════════════════════════════════════════════╣");
