...
//!
//! Parses ATOS checkpoint binary files (raw disk images).

const CHECKPOINT_MAGIC: u32 = 0x41545343; // "ATSC"
...
pub fn parse_and_display(data: &[u8]) {
    ...
    if header_data.len() < SECTOR_SIZE {
        eprintln!("[atos-replay] Data too small for checkpoint header");
        return;
    }
    ...
    if magic != CHECKPOINT_MAGIC {
        eprintln!("[atos-replay] Invalid checkpoint magic: {:#x} (expected ATSC = {:#x})", magic, CHECKPOINT_MAGIC);
        eprintln!("[atos-replay] Tip: use a raw disk image from 'make test' (e.g., /tmp/atos_test.img)");
        return;
    }
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║       ATOS CHECKPOINT ANALYSIS               ║");
    println!("╠══════════════════════════════════════════════╣");
    println!("║ Magic:   ATSC (valid)                       ║");
