...
/// ATOS Execution Proof (mirrors kernel's ExecutionProof)
...
pub fn verify_file(path: &str) {
    let data = match fs::read(path) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("[atos-verify] Failed to read {}: {}", path, e);
            std::process::exit(1);
        }
    };

    if data.len() < PROOF_SIZE {
        eprintln!("[atos-verify] File too small: {} bytes (need {})", data.len(), PROOF_SIZE);
        std::process::exit(1);
    }
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║       ATOS EXECUTION PROOF VERIFICATION      ║");
    println!("╠══════════════════════════════════════════════╣");
