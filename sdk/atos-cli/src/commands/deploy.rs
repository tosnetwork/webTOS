//! atos deploy — send a WASM agent to a running ATOS instance via serial
//!
//! Usage:
//!   atos deploy <agent.wasm> [serial_device]
//!
//! Reads the WASM binary and displays deployment information.
//! Full serial protocol deployment requires the ATOS kernel to support
...
pub fn run(args: &[String]) {
    if args.is_empty() || args[0] == "--help" || args[0] == "-h" {
        println!("Usage: atos deploy <agent.wasm> [serial_device]");
        println!();
        println!("Deploy a WASM agent to a running ATOS instance.");
    ...
    let wasm_bytes = match fs::read(wasm_path) {
        Ok(b) => b,
        Err(e) => {
            eprintln!("[atos-deploy] Failed to read {}: {}", wasm_path, e);
            std::process::exit(1);
        }
    };
    ...
    if wasm_bytes.len() < 8 || &wasm_bytes[0..4] != b"\0asm" {
        eprintln!("[atos-deploy] Invalid WASM binary (bad magic)");
        std::process::exit(1);
    }

    let version = u32::from_le_bytes([wasm_bytes[4], wasm_bytes[5], wasm_bytes[6], wasm_bytes[7]]);

    println!("[atos-deploy] WASM binary: {}", wasm_path);
    println!("[atos-deploy] Size: {} bytes", wasm_bytes.len());
    println!("[atos-deploy] WASM version: {}", version);
    println!("[atos-deploy] Target serial: {}", serial);
    ...
    println!("[atos-deploy] Sections: {}", section_count);

    // Validate size limits
    if wasm_bytes.len() > 65536 {
        eprintln!("[atos-deploy] WARNING: WASM binary exceeds ATOS limit (64 KB)");
        eprintln!("[atos-deploy] ATOS MAX_CODE_SIZE = 65536 bytes");
    }

    println!();
    println!("[atos-deploy] Agent validated. To deploy:");
    println!("  1. Start ATOS with serial: qemu-system-x86_64 -serial stdio ...");
