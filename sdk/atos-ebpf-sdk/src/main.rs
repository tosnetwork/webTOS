//! `atos-ebpf` — ATOS eBPF-lite SDK command-line tool.
//!
//! # Usage
//!
//! ```
//! atos-ebpf compile <input.ebpf> [-o <output.bin>]
//! atos-ebpf verify  <input.bin>
//! atos-ebpf disasm  <input.bin>
//! ```
//!
//! ## compile
...
fn cmd_disasm(args: &[String]) {
    ...
    println!(
        "; ATOS eBPF-lite disassembly of '{}' ({} instruction{})",
        path.display(),
        insns.len(),
        if insns.len() == 1 { "" } else { "s" }
    );
...
fn usage() {
    eprintln!(
        "ATOS eBPF-lite SDK v{}\n\
         \n\
         USAGE:\n\
         \tatos-ebpf compile <input.ebpf> [-o output.bin]   Assemble + verify\n\
         \tatos-ebpf verify  <input.bin>                    Verify binary\n\
         \tatos-ebpf disasm  <input.bin>                    Disassemble binary",
