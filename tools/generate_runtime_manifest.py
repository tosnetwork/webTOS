...
def add_file(entries: OrderedDict[str, str], tos_path: str, host_path: Path) -> None:
        ...
        for raw_line in trace_path.read_text(encoding="utf-8", errors="replace").splitlines():
            for match in path_re.finditer(raw_line):
                ...
                if not path_str.startswith(str(stdlib)):
                    continue
                tos_path = os.path.normpath(path_str)
                path = Path(path_str)
                if path.is_file():
                    results[tos_path] = path.resolve()
    finally:
        try:
            trace_path.unlink()
...
def trace_java_files(java_bin: Path, home: Path) -> list[tuple[str, Path]]:
    with tempfile.NamedTemporaryFile(prefix="tos-java-trace-", delete=False) as trace_file:
        trace_path = Path(trace_file.name)
...
def main() -> int:
    ...
    if "python" in runtimes:
        ...
        for tos_path, host_path in parse_ldd(python_bin):
        ...
        if args.python_stdlib == "full":
            ...
            for tos_path, host_path in trace_python_files(python_bin, stdlib):
                add_file(files, tos_path, host_path)

    if "node" in runtimes:
        node_bin = resolve_executable(args.node)
        add_file(files, "/usr/bin/node", node_bin)
        for tos_path, host_path in parse_ldd(node_bin):
            add_file(files, tos_path, host_path)
    ...
    if "java" in runtimes:
        ...
        if args.java_home == "full":
            ...
            for tos_path, host_path in trace_java_files(java_bin, home):
                add_file(files, tos_path, host_path)
    ...
    for tos_path, host_path in files.items():
        lines.append(f"{tos_path} = {host_path}")
    if files and trees:
        lines.append("")
