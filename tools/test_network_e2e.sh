#!/bin/bash
# TOS Network End-to-End Test
#
# Tests UDP packet send/receive between TOS and a host-side UDP server.
# Uses QEMU user networking with port forwarding.
#
# Architecture:
#   Host (UDP server on port 5555) ←→ QEMU (TOS netd agent, port 4001)
#
# The TOS routerd agent broadcasts HELLO packets on UDP port 4001.
...
# Run TOS with QEMU user networking
# Use virtio-net with user-mode networking (NAT)
echo "[2/4] Launching TOS with virtio-net..."
timeout 8 qemu-system-x86_64 \
    -m "$QEMU_MEMORY" \
    -serial file:/tmp/tos_net_e2e.log \
    -display none \
    -kernel "$ELF32" \
...
BOOT_OK=$(grep -ac "TOS boot ok" /tmp/tos_net_e2e.log 2>/dev/null || echo 0)
NIC_OK=$(grep -ac "Initialized\|initialized" /tmp/tos_net_e2e.log 2>/dev/null || echo 0)
NETD_OK=$(grep -ac "Netd agent created\|e1000.*Initialized\|VIRTIO.*Initialized" /tmp/tos_net_e2e.log 2>/dev/null || echo 0)
...
PACKET_SENT=$(grep -c "Test packet sent\|packet sent\|UDP.*send" /tmp/tos_net_e2e.log 2>/dev/null || echo 0)
echo "  Packet:  $([ "$PACKET_SENT" -ge 1 ] && echo 'PASS (packet sent)' || echo 'no send logged')"

# Show network-related kernel log
echo ""
echo "=== Network log ==="
grep -a "VIRTIO\|e1000\|netd\|NETD\|routerd\|ROUTERD\|NIC\|packet\|UDP" /tmp/tos_net_e2e.log 2>/dev/null | head -10
