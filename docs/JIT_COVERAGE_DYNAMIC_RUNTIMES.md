...
$ GUEST_MEM_MB=3072 \
  ...
  jit_coverage ~/.nvm/versions/node/v24.13.0/bin/node \
    -e "let s=0; for(let i=0;i<20000000;i++){ s+=Math.sqrt(i)+i*1.5; } console.log(s);"
<RESULTS PENDING — the run is heavy; V8 compiles the loop and its generated
 code is itself profiled by the emulator, so this is the JIT-in-JIT case>
```

## Verdict — pending the compute-loop number, but already clear on trend
