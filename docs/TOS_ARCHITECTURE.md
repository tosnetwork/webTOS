# TOS Architecture: Execution Engine + Settlement Layer

## What TOS Is

TOS is a **bare-metal verifiable execution engine** — it has the hardware control of an OS but operates under the trust model of a smart contract platform.
...
> TOS is a from-scratch, VM-first, AI-native minimal operating system built around agents, mailboxes, capabilities, structured state, execution budgets, and auditable kernel behavior.

---

## TOS vs Smart Contract Platforms vs Traditional OS

| Concept | Ethereum | TOS | Traditional OS |
|---------|----------|-----|----------------|
...
Energy in TOS behaves like rocket fuel: it is consumed by execution and cannot be recovered. It is **not** a token, not a coin, and not transferable outside the system.
...
**TOS cannot prevent a node from refusing to execute or withholding results.** This requires a consensus layer. But TOS can guarantee that any submitted result is verifiable.

---

## TOS + gtos: Execution Engine + Settlement Layer

TOS does computation. gtos (blockchain) does notarization. Each does what it is best at.
...
| **Execution** | TOS direct | **milliseconds** | Every request |
...
Step 2: EXECUTE (TOS direct, milliseconds, repeated)
  User -> TOS nodeA: "Run this inference task"
  TOS bridge_agent: validates credential, checks energy balance
  TOS kernel: creates Agent, executes (~10ms), charges energy
  TOS -> User: returns result directly (no chain involved)

Step 3: SETTLE (on-chain, batched)
  TOS nodeA -> gtos: ATOSReceiptAnchor.submitReceipt(hash, stateRoot, 48320)
  gtos -> gtos: ATOSEnergySettlement.settle(hash, nodeA, 48320)
  nodeA.balance -= 48320
```

After the initial deposit, the user interacts **directly with TOS** for all execution. This is essential — if every AI inference call had to go through a blockchain transaction, the system would be unusable.

### Analogy

This is identical to how transit cards work:

| Transit Card | TOS + gtos |
|-------------|------------|
| Top up card at machine (slow, once) | Deposit TOS on gtos (slow, once) |
| Tap card on bus (fast, every ride) | Call TOS API (fast, every request) |
| Transit company reconciles with bank (daily batch) | TOS submits receipts to gtos (periodic batch) |

---

## Payment Agnosticism

TOS is a **payment-agnostic execution engine**. It sells compute (energy), not a specific token. What can buy energy is determined by the settlement adapter layer outside the kernel.
...
Registers TOS execution nodes on-chain with their attestation hashes and public keys. Tracks reputation and allows slashing of dishonest nodes.

### ATOSReceiptAnchor.tol

Stores TOS execution receipt hashes on-chain for immutable audit trails. Supports dispute resolution — anyone with a replay proof can challenge a receipt.

### ATOSEnergySettlement.tol

Manages energy credit deposits and consumption settlement. External payers deposit tokens, TOS nodes draw energy, and receipts prove consumption for billing.

---

## From VM to RM: TOS as a Hardware-Native Ethereum
...
TOS takes the same computational model and turns it into a **Real Machine** — an operating system that runs directly on x86_64 hardware, drives its own devices, and executes agents on its own schedule.

```
EVM: Smart contract bytecode → runs inside geth → runs inside Linux → runs on hardware
TOS: Agent code (WASM/native) → runs on TOS kernel → runs on hardware (direct)
```

### Structural Isomorphism

The conceptual mapping between EVM and TOS is nearly 1:1:

| EVM (Virtual Machine) | TOS (Real Machine) | Why It Matters |
|---|---|---|
...
| Genesis Block | TOS Genesis (§4.5) | Both bootstrap the initial authority, state, and budget |
...
### What TOS Adds Beyond EVM

The EVM model has fundamental limitations that TOS overcomes by being a real operating system:

#### 1. Hardware Control

EVM has zero hardware access — it runs inside a process on a host OS. TOS drives hardware directly:
...
TOS agents are **active** — the kernel's timer interrupt drives preemptive scheduling. Agents can:
...
This is the difference between a **stored procedure** (EVM) and a **running process** (TOS).

#### 3. Multiple Runtime Backends

EVM supports exactly one bytecode format (EVM opcodes). TOS supports three:
...
TOS uses **asynchronous mailbox messaging** — agent A sends a message to agent B's mailbox and continues (or blocks waiting for a reply). There is no shared call stack. This eliminates reentrancy by design and enables natural concurrency patterns.
...
TOS supports **SMP multi-core** scheduling — multiple agents execute in parallel on different CPU cores, with spinlock-protected shared kernel structures. Cross-core communication uses the same mailbox IPC.
...
TOS has **system agents as resource brokers**:
...
TOS takes that proven model and asks: what if the execution engine **was** the operating system? What if it could drive its own hardware, schedule its own agents, manage its own storage, and produce its own verifiable receipts — without depending on a host OS or requiring global consensus for every computation?
...
TOS's path:        Hardware → TOS kernel → Agent (direct)
```
...
| Question | Answer |
|----------|--------|
| Is TOS a blockchain? | No. It does not replicate execution across nodes. |
| Is TOS a traditional OS? | No. It enforces capability-scoped authority and energy budgets at the kernel level. |
| What is TOS? | A bare-metal verifiable execution engine for autonomous agents. |
...
| Where is the entry point? | Deposit on gtos (once), then interact directly with TOS (milliseconds). |
| What does gtos do? | Notarization: stores receipt hashes, manages deposits, handles disputes. |
| What does TOS do? | Execution: runs agents at bare-metal speed, produces verifiable receipts. |
