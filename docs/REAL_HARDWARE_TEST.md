# TOS Real Hardware Testing Guide

This document describes how to boot TOS on a physical x86_64 machine using the UEFI boot path. TOS was developed VM-first, so real hardware testing requires extra steps. Follow this guide carefully.
...
- **RAM:** 512 MB minimum (TOS identity-maps the first 512 MB). 1 GB or more is fine.
- **Storage:** Any USB 2.0 or 3.0 flash drive (1 GB minimum) to boot from.
- **Serial port (recommended):** DB9 RS-232 COM1 port, or a USB-to-serial adapter (FTDI chip preferred). Required to see any output — TOS has no video driver and produces no screen output.
- **USB keyboard:** Required to interact with BIOS/UEFI firmware menus only. TOS itself has no keyboard driver.

### Build Host Requirements

The build host (where you compile TOS) must have:
...
```bash
make uefi-build
# Confirm: uefi/target/x86_64-unknown-uefi/release/tos-uefi.efi exists
ls -lh uefi/target/x86_64-unknown-uefi/release/tos-uefi.efi
```
...
TOS boots as a standard UEFI application placed at `EFI/BOOT/BOOTX64.EFI` on a FAT32 ESP (EFI System Partition).
...
sudo mkdir -p /mnt/tos-usb
sudo mount /dev/sdX1 /mnt/tos-usb

# Create the UEFI boot path
sudo mkdir -p /mnt/tos-usb/EFI/BOOT

# Copy the TOS UEFI loader
sudo cp uefi/target/x86_64-unknown-uefi/release/tos-uefi.efi \
        /mnt/tos-usb/EFI/BOOT/BOOTX64.EFI

# Verify
ls -lh /mnt/tos-usb/EFI/BOOT/BOOTX64.EFI

# Unmount cleanly
sudo umount /mnt/tos-usb
sync
```
...
Secure Boot will refuse to run TOS because the `.efi` binary is not signed by a trusted certificate authority.
...
TOS has **no video driver**. The screen will either stay blank or show only the UEFI firmware splash. All TOS output goes to COM1 serial at 115200 baud 8N1.

If you do not have a serial connection attached yet, proceed anyway. The machine will either:
- Boot silently (TOS is running but you cannot see output), or
- Hang at a black screen (a failure occurred before the kernel halted).

### Verifying a Successful Boot Without Serial

A successful UEFI boot with no serial cable shows no visible activity after the firmware splash disappears. The CPU will be executing the TOS kernel idle loop. If the machine reboots in a loop or shows a UEFI "Boot Manager" error screen, something went wrong — see Section 6 (Troubleshooting).
...
```
[UEFI] TOS UEFI boot loader starting
...
If you see all of the above, TOS is running correctly on real hardware.
...
If the test machine has no DB9 port: TOS does not support USB serial. COM1 (port 0x3F8) must be a physical 16550-compatible UART. Check the motherboard manual — many desktop boards with no external DB9 connector still have a COM1 header on the PCB (9-pin shrouded header). A DB9 bracket cable can expose it.
...
```bash
picocom -b 115200 /dev/ttyUSB0 --logfile /tmp/tos-boot-$(date +%Y%m%d-%H%M%S).txt
```

This saves everything to a timestamped file for later inspection with `sdk/tos-cli`:

```bash
cd sdk/tos-cli
cargo build --release
./target/x86_64-unknown-linux-gnu/release/tos inspect /tmp/tos-boot-*.txt
```
...
### Kernel panics after "TOS boot ok"

**Cause:** A hardware feature assumed by TOS is absent or behaves differently than on QEMU.
...
- [ ] Serial output shows `TOS boot ok` and agent startup messages
...
- [ ] `[UEFI] TOS UEFI boot loader starting` appears
...
- [ ] Log analyzed with `tos inspect` from the SDK CLI
...
TOS has no framebuffer driver, no VGA driver, and no GOP (Graphics Output Protocol) initialization. The screen remains blank or shows only the firmware splash. All output is COM1 serial only.

### No USB Input

TOS has no USB HID driver. Keyboards and mice connected via USB are non-functional inside TOS. The only input path is serial, and TOS currently has no interactive serial shell — serial is output-only.
...
TOS does not configure the IOMMU. DMA from devices with an IOMMU-enforcing firmware (VT-d enabled) may be blocked or fault silently.

### No Power Management

TOS has no ACPI power management, no sleep states, and no thermal management. The machine will run at full power until physically powered off. There is no `shutdown` or `reboot` command — power off the test machine using its physical power button.

### Hardware Watchdog

Some enterprise machines have a hardware watchdog that the BIOS arms and expects the OS to service. TOS does not service any watchdog. If the target machine has an armed watchdog, it will reset the machine after a timeout (commonly 60–300 seconds). Disable the watchdog in BIOS if present, or look for a **Watchdog Timer** setting under the **Advanced** or **Server Management** tab.
